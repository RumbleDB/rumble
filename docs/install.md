...
After successful completion, you can check the `target` directory, which should contain the compiled classes as well as the JAR file `spark-rumble-1.3-jar-with-dependencies.jar`.
    
## Running locally

The most straightforward to test if the above steps were successful is to run the Rumble shell locally, like so:

    $ spark-submit --master local[2] --deploy-mode client target/spark-rumble-1.3-jar-with-dependencies.jar --shell yes
...
You can also try to run the Rumble shell on a cluster if you have one available and configured -- this is done in the same way as any other `spark-submit` command:
    ...
    $ spark-submit --master yarn --deploy-mode client --num-executors 40 spark-rumble-1.3.jar

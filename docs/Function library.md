...
Rumble namespace, which is the default namespace and does not require any prefix in front of function names.

It is recommended that user-defined functions are put in the local namespace, i.e., their name should have the local: prefix. Otherwise, there is the risk that your code becomes incompatible with subsequent releases if new
builtin functions are introduced.

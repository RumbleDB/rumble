...
let $users := json-file("./datasets/users.json")
let $items := json-file("./datasets/items.json")
let $bids := json-file("./datasets/bids.json")

let $q1_users := copy $copy_users := [$users]
                 modify append {"userID" : "U07", "name" : "Annabel Lee"} into $copy_users
                 return $copy_users[]

let $q2_bids := copy $copy_bids := [$bids]
                modify append {"userID" : "U07", "itemNO" : 1001, "bid" : 60, "bid_date" : "1999-02-01"} into $copy_bids
                return $copy_bids[]

let $q3_bids := copy $copy_bids := [$q2_bids], $uid := $q1_users[$$.name eq "Annabel Lee"].userID, $top_bid := max($q2_bids[$$.itemNO eq 1002].bid)
                modify append {"userID" : $uid, "itemNO" : 1002, "bid" : $top_bid * 1.1, "bid_date" : "1999-02-01"} into $copy_bids
                return $copy_bids[]
...
let $q5_bids := copy $copy_bids := [$q3_bids], $uid := $q4_users[$$.name eq "Annabel Lee"].userID, $top_bid := max($q3_bids[$$.itemNO eq 1007].bid)
                modify
                    ...
                    then
                        append {"userID" : $uid, "itemNO" : 1007, "bid" : $top_bid * 1.1, "bid_date" : "1999-02-01"} into $copy_bids
                    else
                        ()
                return $copy_bids[]

...
public class CLIArgumentParserTest {
    ...
    public void runRequiresAQuerySource() {
        Assertions.assertThrows(
                CommandLine.MissingParameterException.class, () -> CLIArgumentParser.parse("run"));
    }
    ...
    public void runMapsConfigurationArguments() {
        ...
        Assertions.assertFalse(
                configuration.optimization().optimizeGeneralComparisonToValueComparison());
        ...
        Assertions.assertEquals(
                "https://example.com/base/", configuration.semantics().staticBaseUri());

        Assertions.assertEquals(
                "Europe/Madrid", configuration.formatting().defaultFormattingPlace());
    ...
    public void conflictingQuerySourcesAreRejected() {
        ...
        Assertions.assertThrows(
                ...
                () ->
                        CLIArgumentParser.parse(
                                "run", "--query", "1 + 1", "--query-path", "queries/main.jq"));
        Assertions.assertThrows(
                CommandLine.ParameterException.class,
    ...
    public void bindingsRemainSeparateFromConfiguration() {
        ...
        LexicalBinding answer =
                bindings.get(Name.createVariableInNoNamespace("answer"), LexicalBinding.class)
                        .orElseThrow();
        FileBinding payload =
                bindings.get(Name.createVariableInNoNamespace("payload"), FileBinding.class)
                        .orElseThrow();
    ...
    public void literalContextItemIsSupported() {
        CLIInvocation invocation =
                CLIArgumentParser.parse("run", "--query", "1", "--context-item", "{\"a\":1}");

        LexicalBinding contextItem =
                invocation.bindings().get(Name.CONTEXT_ITEM, LexicalBinding.class).orElseThrow();
    ...
    public void invalidTypedValuesAreRejected() {
        ...
        Assertions.assertThrows(
                ...
                () ->
                        CLIArgumentParser.parse(
                                "run", "--query", "1", "--default-formatting-place", "Not/AZone"));
        Assertions.assertThrows(
                CliException.class,
                () ->
                        CLIArgumentParser.parse(

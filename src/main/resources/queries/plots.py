...
def breakEven(logDir, files, names, title, breakEvenSizes, oneSize, partIt = ["-300-45"], plotZorba = False, divisor = 1000000, iterations = 5):
    #break even analysis   
    partitions = 300
    ...
    for currentFile in files:
        ...
        for size in breakEvenSizes:
            ...
            if size == 1:
                ...
                sizes.append(size/divisor)
                zorbaSizes.append(size/divisor)
                
                
            rawResults = []
            for iteration in range(iterations):
                filePath = logDir + 'LOG-' + currentFile + str(size) + partIt[0]  +'_IT' + \
                            str(iteration)
                if not os.path.isfile(filePath) and len(partIt) > 1:
                    filePath = logDir + 'LOG-' + currentFile + str(size) + partIt[1]  +'_IT' + \
                            str(iteration)
            ...
            #plot
        plt.xlabel('#billions of JSON objects', fontsize=12)
        plt.ylabel('Runtime in s', fontsize=12)
        plt.tick_params(axis='both', which='major', labelsize=12)
        plt.tick_params(axis='both', which='minor', labelsize=12)
        axes = plt.gca()
        #axes.set_xlim([200,1650])
        #axes.set_ylim([0,350])
...
def perfromanceWithStdDeviation(plotSpeedup, logDir, fileNames, size, partitions, iterations, \
    ...
    for currentFile in fileNames:
                ...
                with open(currentFilePath, "r") as ins:
                    for line in ins:
                        if line.startswith("[ExecTime]"):
                            ...
                            rawResults.append(time)
                            if executorNumber > 1:
                                rawSpeeds.append((serialTime * 1.0)/time)
                            else :
                                rawSpeeds.append(time/time)
            if executorNumber == 1:
                serialTime = numpy.mean(rawResults)
                serialSTD = numpy.std(rawResults)
            times.append(numpy.mean(rawResults))
            stds.append(numpy.std(rawResults))
            speeds.append(numpy.mean(rawSpeeds))
            speedsSTD.append(numpy.std(rawSpeeds))
        ...
        if plotSpeedup:
            ...
            #axes.set_ylim([0,450])
            plt.suptitle(title, fontsize=12)
            plt.errorbar(units, speeds, yerr=speedsSTD, marker='s', label='Speedups for 16 million objects',markersize=7)
            legend = plt.legend(loc='upper c', shadow=True)  
            plt.grid('on')
            plt.show()
...
files = ['Where-RedditS']
perfromanceWithStdDeviation(False, directory,files, 1, 500, 5, \
 [10, 15, 20,30,40,50,60,75,100], "#executors", "Runtime in s", \
 "Number of executors - performance analysis")    
 
 
directory = '/home/stefan/Work/ETH/Thesis/benchmarks/time/speedup/'
files = ['Where-Confusion']
perfromanceWithStdDeviation(True, directory,files, 1, 300, 5, \
 [1,3 ,5, 10, 15, 20, 30, 40, 50, 60, 75, 100, 125], "#executors", "Runtime in s", \
 "Number of executors - performance analysis")    
...
directory = '/home/stefan/Work/ETH/Thesis/benchmarks/time/big-reddit/'
files = ['Where-Reddit']
names = ['Filter-only']
breakEvenSizes = [200, 864, 1390, 1600,3200, 4800, 6400]#, 1]     

breakEven(directory, files, names, "Scalability " , breakEvenSizes, 14, ["-3000-550", "-15000-550"], False, 1000, 10)   

files = ['WhereGroupBy-Reddit']
names = ['Grouping ']
breakEvenSizes = [200, 864, 1390, 1600,3200, 4800]#, 1]     

breakEven(directory, files, names, "Scalability " , breakEvenSizes, 14, ["-3000-550", "-15000-550"], False, 1000, 5) 
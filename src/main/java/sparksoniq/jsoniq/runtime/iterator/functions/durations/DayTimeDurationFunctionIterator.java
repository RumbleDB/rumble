...
public class DayTimeDurationFunctionIterator extends LocalFunctionCallIterator {
    ...
    public Item next() {
        if (this._hasNext) {
            this._hasNext = false;
            try {
                Period period = DurationItem.getDurationFromString(_durationStringItem.getStringValue(), AtomicTypes.DayTimeDurationItem);
                ...
                String message = String.format("\"%s\": value of type %s is not castable to type %s",
                        _durationStringItem.serialize(), "string", "dayTimeDuration");
    ...
    public static class YearMonthDurationFunctionIterator extends LocalFunctionCallIterator {

    ...
    public void open(DynamicContext context) {
        ...
        try {
            ...
            throw new UnexpectedTypeException(
                    e.getJSONiqErrorMessage() + "? of function months-from-duration()",
                    this._children.get(0).getMetadata()
            );
        } catch (UnknownFunctionCallException e) {
            throw new UnexpectedTypeException(
                    " Sequence of more than one item can not be promoted to parameter type duration? of function months-from-duration()",
                    getMetadata()
            );
        }

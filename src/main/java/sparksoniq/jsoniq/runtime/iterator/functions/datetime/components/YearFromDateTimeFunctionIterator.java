    ...
    public void open(DynamicContext context) {
        ...
        try {
            ...
            throw new UnexpectedTypeException(
                    e.getJSONiqErrorMessage() + "? of function year-from-dateTime()",
                    this._children.get(0).getMetadata()
            );
        } catch (UnknownFunctionCallException e) {
            throw new UnexpectedTypeException(
                    " Sequence of more than one item can not be promoted to parameter type dateTime? of function year-from-dateTime()",
                    getMetadata()
            );
        }

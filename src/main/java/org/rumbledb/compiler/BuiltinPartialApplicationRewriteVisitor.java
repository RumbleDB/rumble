...
/**
 * Rewrites direct partial application of builtins, e.g. {@code fn:max(0, ?)}, into an equivalent
 * inline function, e.g. {@code function($x) { fn:max(0, $x) }}.
 *
 * <p>This keeps builtin partial application on the same code path as ordinary inline functions.
 */
public class BuiltinPartialApplicationRewriteVisitor extends CloneVisitor {
    ...
    private InlineFunctionExpression rewriteBuiltinPartialApplication(
        ...
        FunctionCallExpression bodyCall =
                new FunctionCallExpression(
                        functionName, fullArguments, sourceExpression.getMetadata());
        StatementsAndOptionalExpr body =
                new StatementsAndOptionalExpr(
                        Collections.emptyList(), bodyCall, sourceExpression.getMetadata());
    ...
    public Node visitFunctionCall(FunctionCallExpression expression, Node argument) {
        ...
        if (!expression.isPartialApplication() || builtin == null) {
            // In case of non-partial application or non-builtin function, we still need to keep
            // descending Because a
        ...
        return rewriteBuiltinPartialApplication(
                expression.getFunctionName(), builtin, arguments, expression);
    }
    ...
    public Node visitDynamicFunctionCallExpression(
        ...
        List<Expression> arguments =
                expression.getArguments().stream()
                        .map(expr -> expr != null ? (Expression) visit(expr, argument) : null)
                        .collect(Collectors.toList());
        Expression rewrittenMainExpression =
                (Expression) visit(expression.getMainExpression(), argument);

        if (!(rewrittenMainExpression
                instanceof NamedFunctionReferenceExpression namedFunctionReference)) {

...
/**
 * Parses XQuery 3.1 {@code URIQualifiedName} tokens ({@code BracedURILiteral NCName}) into expanded
 * {@link Name} values. See <a
 * href="https://www.w3.org/TR/xquery-31/#doc-xquery31-URIQualifiedName">XQuery 3.1</a>.
 */
public final class URIQualifiedNameParser {
    ...
    /**
     * @param tokenText full lexer text of one {@code URIQualifiedName} token, e.g. {@code
     *     Q{http://ex}invoice}
    ...
    public static Name parse(String tokenText, ExceptionMetadata metadata) {
        ...
        if (closeBrace < 0) {
            throw new ParsingException(
                    "Invalid URIQualifiedName (no closing '}') : " + tokenText, metadata);
        }
        String uriRaw = tokenText.substring(2, closeBrace);
        String localName = tokenText.substring(closeBrace + 1);
        if (localName.isEmpty()) {
            throw new ParsingException(
                    "Invalid URIQualifiedName (missing local name): " + tokenText, metadata);
        }
    ...
    /**
     * Whitespace normalization in the spirit of {@code xs:anyURI} (collapse, trim). Empty content
     * after processing means absent namespace URI (e.g. {@code Q{}local}).
     */

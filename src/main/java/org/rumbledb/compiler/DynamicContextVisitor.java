...
public class DynamicContextVisitor extends AbstractNodeVisitor<DynamicContext> {
    ...
    @Override
    public DynamicContext visitFunctionDeclaration(
            FunctionDeclaration declaration, DynamicContext argument) {
        ...
        RuntimeIterator bodyIterator =
                VisitorHelpers.generateRuntimeIterator(expression, this.configuration);
    ...
    // @Override
    // public DynamicContext visitTransformExpression(TransformExpression expression, DynamicContext
    // argument) {
    //
    ...
    @Override
    public DynamicContext visitVariableDeclaration(
            VariableDeclaration variableDeclaration, DynamicContext argument) {
            ...
            if (!sequenceType.equals(SequenceType.createSequenceType("()"))
                ...
                URI resolvedURI =
                        FileSystemUtil.resolveURIAgainstWorkingDirectory(
                                value, ExceptionMetadata.EMPTY_METADATA);
                ...
                if (!InstanceOfIterator.doesItemTypeMatchItem(itemType, item)) {
                    Item castItem =
                            CastIterator.castItemToType(
                                    item, itemType, variableDeclaration.getMetadata());
                    if (castItem == null) {
                        throw new UnexpectedTypeException(
                                "External variable value ("
                                        + item.serialize()
        ...
        // Variable is external and we do not have any supplied value: we fall back to expression,
        // if
        // any.
        Expression expression = variableDeclaration.getExpression();
        if (expression != null) {
    ...
    @Override
    public DynamicContext visitTypeDeclaration(
            TypeDeclaration declaration, DynamicContext argument) {
    ...
    public DynamicContext visitLibraryModule(LibraryModule module, DynamicContext argument) {
        if (!this.importedModuleContexts.containsKey(module.getNamespace())) {
            DynamicContext newContext =
                    new DynamicContext(this.configuration, this.externalBindings);
    ...
    private String getFileValueBinding(Name name, ExceptionMetadata metadata) {
        ...
        try {
            URI uri =
                    FileSystemUtil.resolveURIAgainstWorkingDirectory(
                            fileBinding.getLocation(), metadata);
    ...
    private String getInputFormat(Name name) {
        if (this.externalBindings.get(name, StandardInputBinding.class).isPresent()) {
            InputFormat format =
                    this.externalBindings.get(name, StandardInputBinding.class).get().getFormat();

...
public class ExecutionModeVisitor extends AbstractNodeVisitor<StaticContext> {
    ...
    public StaticContext visitFunctionDeclaration(
        ...
        expression.setHighestExecutionMode(ExecutionMode.LOCAL);
        declaration.setHighestExecutionMode(
                expression.getBody().getHighestExecutionMode(this.visitorConfig));
        expression.registerUserDefinedFunctionExecutionMode(this.visitorConfig);
    ...
    public StaticContext visitInlineFunctionExpr(
        ...
        expression
                .getParams()
                .forEach(
                        (paramName, sequenceType) ->
                                expression
                                        .getBody()
                                        .getStaticContext()
                                        .setVariableStorageMode(paramName, ExecutionMode.LOCAL));
        // visit the body first to make its execution mode available while adding the function to
        // the
        // catalog
    ...
    public StaticContext visitForClause(ForClause clause, StaticContext argument) {
        ...
        argument.setVariableStorageMode(
                clause.getVariableName(), clause.getVariableHighestStorageMode(this.visitorConfig));

        if (clause.getPositionalVariableName() != null) {
            argument.setVariableStorageMode(
                    clause.getPositionalVariableName(), ExecutionMode.LOCAL);
        }
        return argument;
    }
    ...
    public StaticContext visitLetClause(LetClause clause, StaticContext argument) {
        ...
        // if let clause is local, defined variables are stored according to the execution mode of
        // the
        // expression
        if (clause.getHighestExecutionMode(this.visitorConfig) == ExecutionMode.LOCAL) {
            clause.setVariableHighestExecutionMode(
                    clause.getExpression().getHighestExecutionMode(this.visitorConfig));
        } else {
    ...
    public StaticContext visitGroupByClause(GroupByClause clause, StaticContext argument) {
        ...
        for (Entry<Name, InScopeVariable> entry : argument.getInScopeVariables().entrySet()) {
            ...
            argument.setVariableStorageMode(
                    entry.getKey(), clauseContext.getVariableStorageMode(entry.getKey()));
        }
        return argument;
    }
    ...
    public StaticContext visitTypeSwitchExpression(
        ...
        for (TypeswitchCase c : expression.getCases()) {
            ...
            if (variableName != null) {
                c.getReturnExpression()
                        .getStaticContext()
                        .setVariableStorageMode(variableName, ExecutionMode.LOCAL);
            }
        ...
        for (TypeswitchCase c : expression.getCases()) {
            ExecutionMode mode =
                    c.getReturnExpression().getHighestExecutionMode(this.visitorConfig);
            if (mode.isUnset()) {
                expression.setHighestExecutionMode(ExecutionMode.UNSET);
                return argument;
            }
    ...
    public StaticContext visitSwitchExpression(
        ...
        ExecutionMode defaultMode =
                expression.getDefaultExpression().getHighestExecutionMode(this.visitorConfig);
    ...
    public StaticContext visitVariableDeclaration(
        ...
        Arity arity =
                argument.getVariableSequenceType(variableDeclaration.getVariableName()).getArity();
    ...
    public StaticContext visitRangeExpr(RangeExpression rangeExpression, StaticContext argument) {
        ...
        if (left instanceof IntegerLiteralExpression leftLiteralExpr
            ...
            if (rightValue
                            .subtract(leftValue)
                            .compareTo(BigInteger.valueOf(RangeOperationIterator.PARTITION_SIZE))
                    >= 0) {
    ...
    public StaticContext visitSimpleMapExpr(
            SimpleMapExpression simpleMapExpression, StaticContext argument) {
    ...
    public StaticContext visitFilterExpression(
        ...
        visitDescendants(expression, argument);
        if (expression.getPredicateExpression()
                instanceof IntegerLiteralExpression intLiteralExpr) {
    ...
    public StaticContext visitReturnStatementClause(
            ReturnStatementClause statement, StaticContext argument) {
    ...
    public StaticContext visitStatementsAndOptionalExpr(
        ...
        if (statementsAndOptionalExpr.getExpression() != null) {
            ExecutionMode exprExecutionMode =
                    statementsAndOptionalExpr
                            .getExpression()
                            .getHighestExecutionMode(this.visitorConfig);
            ExecutionMode mergedExecutionMode =
                    getHighestExecutionMode(statementsExecMode, exprExecutionMode);
    ...
    public StaticContext visitStatementsAndExpr(
        ...
        ExecutionMode statementsExecMode =
                getHighestExecutionModeFromStatements(statementsAndExpr.getStatements());
        ExecutionMode exprExecutionMode =
                statementsAndExpr.getExpression().getHighestExecutionMode(this.visitorConfig);
        ExecutionMode mergedExecutionMode =
                getHighestExecutionMode(statementsExecMode, exprExecutionMode);
    ...
    public StaticContext visitNodeSetExpr(NodeSetExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ExecutionMode leftMode =
                expression.getLeftExpression().getHighestExecutionMode(this.visitorConfig);
        ExecutionMode rightMode =
                expression.getRightExpression().getHighestExecutionMode(this.visitorConfig);
    ...
    public StaticContext visitExitStatement(ExitStatement exitStatement, StaticContext argument) {
        ...
        exitStatement.setHighestExecutionMode(
                exitStatement.getExitExpression().getHighestExecutionMode());
        this.exitStatementChildren.add(exitStatement);
        return argument;
    }

    private static ExecutionMode getHighestExecutionMode(
            ExecutionMode firstExecMode, ExecutionMode secondExecMode) {

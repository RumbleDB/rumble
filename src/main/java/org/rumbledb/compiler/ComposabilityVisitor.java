...
/**
 * Visitor checks rules for composing statements based on sequential properties or nesting (e.g.,
 * break statements). The visitor passes a node representing the innermost control statement (while
 * or FLWOR), and for some of the rules applies checks to verify if loops enclose the visited node
 * type.
 */
public class ComposabilityVisitor extends AbstractNodeVisitor<Node> {
    ...
    public Node visitProlog(Prolog expression, Node argument) {
        expression
                ...
                .forEach(functionDeclaration -> visit(functionDeclaration, null));
        expression
                .getVariableDeclarations()
                .forEach(variableDeclaration -> visit(variableDeclaration, null));
        boolean containsOtherThanNonUpdatingNonSeq =
                expression.getVariableDeclarations().stream()
                        .anyMatch(
                                varDecl -> {
    ...
    public Node visitStatementsAndOptionalExpr(
        ...
        if (statementsAndOptionalExpr.getStatements() != null) {
            statementsAndOptionalExpr
                    .getStatements()
                    .forEach(statement -> visit(statement, argument));
        }

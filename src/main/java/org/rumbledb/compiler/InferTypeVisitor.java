...
public class InferTypeVisitor extends AbstractNodeVisitor<StaticContext> {
    ...
    private void throwStaticTypeException(
            String message, ErrorCode code, ExceptionMetadata metadata) {
        if (this.rumbleRuntimeConfiguration.analysis().enableStaticTyping()) {
    ...
    private SequenceType requireInferredType(SequenceType type, String nodeName) {
        if (type == null) {
            throw new OurBadException(
                    "A child expression of a " + nodeName + " has no inferred type");
        }
        return type;
    }

    /**
     * Perform basic checks on a list of SequenceType, available checks are for null (OurBad
     * exception) and inferred the empty sequence (XPST0005)
     *
    ...
    /**
     * Perform basic checks on a SequenceType, available checks are for null (OurBad exception) and
     * inferred the empty sequence (XPST0005)
     *
    ...
    public StaticContext visitCommaExpression(CommaExpression expression, StaticContext argument) {
        ...
        for (Expression childExpression : expression.getExpressions()) {
            ...
            // if the child expression is an EMPTY_SEQUENCE it does not affect the comma expression
            // type
            if (!childExpressionInferredType.isEmptySequence()) {
                if (inferredType.isEmptySequence()) {
                    ...
                    if (inferredType.getItemType().isObjectItemType()
                        ...
                        resultingItemType =
                                (currentKeys.size() == childKeys.size()
                                                && currentKeys.stream()
                                                        .allMatch(
                                                                key ->
                                                                        childKeys.contains(key)
                                                                                && currentItemType
                    ...
                    SequenceType.Arity resultingArity =
                            ((inferredType.getArity() == SequenceType.Arity.OneOrZero
                                                    || inferredType.getArity()
                                                            == SequenceType.Arity.ZeroOrMore)
                                            && (childExpressionInferredType.getArity()
    ...
    public StaticContext visitVariableReference(
        ...
        if (variableType == null) {
            // if is null, no 'as [SequenceType]' part was present in the declaration, therefore we
            // infer
            // it
            variableType =
                    expression
                            .getStaticContext()
                            .getVariableSequenceType(expression.getVariableName());
    ...
    @Override
    public StaticContext visitArrayConstructor(
            ArrayConstructorExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitObjectConstructor(
            ObjectConstructorExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        if (expression.isMergedConstructor()) {
            // if it is a merged constructor the child must be a subtype of object* inferred type
            SequenceType childSequenceType =
                    requireInferredType(
    ...
    @Override
    public StaticContext visitMapConstructor(
            MapConstructorExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitDirPIConstructor(
            DirPIConstructorExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(
                new SequenceType(BuiltinTypesCatalogue.processingInstructionNode));
        return argument;
    }
    ...
    @Override
    public StaticContext visitTextNodeConstructor(
            TextNodeConstructorExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitAttributeNode(
            AttributeNodeExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(new SequenceType(BuiltinTypesCatalogue.attributeNode));
        return argument;
    }
    ...
    @Override
    public StaticContext visitAttributeNodeContent(
            AttributeNodeContentExpression expression, StaticContext argument) {
    ...
    @Override
    public StaticContext visitContextExpr(
            ContextItemExpression expression, StaticContext argument) {
    ...
    public StaticContext visitInlineFunctionExpr(
        ...
        FunctionSignature signature =
                new FunctionSignature(params, returnType, expression.isUpdating());
        expression.setStaticSequenceType(
                new SequenceType(ItemTypeFactory.createFunctionItemType(signature)));
        return argument;
    }

    private FunctionSignature getSignature(
            FunctionIdentifier identifier, StaticContext staticContext) {
        BuiltinFunction function = null;
        FunctionSignature signature = null;
        function =
                BuiltinFunctionCatalogue.getBuiltinFunction(
                        identifier, staticContext.getQueryLanguage());
        if (function != null) {
    ...
    @Override
    public StaticContext visitNamedFunctionRef(
            NamedFunctionReferenceExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        try {
            FunctionSignature signature =
                    getSignature(expression.getIdentifier(), expression.getStaticContext());
            expression.setStaticSequenceType(
                    new SequenceType(ItemTypeFactory.createFunctionItemType(signature)));
    ...
    private SequenceType inferStrictAggregateReturnType(
            FunctionCallExpression expression, Expression inputExpression) {
        SequenceType inputType =
                validateStrictAggregateInputType(expression, inputExpression, "fn:avg");
        if (inputType.isEmptySequence()) {
        ...
        SequenceType.Arity returnArity =
                (inputType.getArity() == SequenceType.Arity.One
     ...
     */
    private boolean tryAnnotateSpecificFunctions(
        ...
        if (functionName.equals(Name.createVariableInDefaultFunctionNamespace("parquet-file"))
            ...
            try {
                ...
                expression.setStaticSequenceType(
                        new SequenceType(schemaItemType, SequenceType.Arity.ZeroOrMore));
                return true;
            } catch (Exception e) {
                return false;
            }
        ...
        if (isBuiltinFunctionName(functionName, "sum")) {
            SequenceType inputType =
                    validateStrictAggregateInputType(expression, args.get(0), "fn:sum");
            expression.setStaticSequenceType(
                    new SequenceType(
                            inputType.getItemType(),
                            inputType.getArity() == SequenceType.Arity.OneOrMore
        ...
        if (isBuiltinFunctionName(functionName, "avg")) {
            expression.setStaticSequenceType(
                    inferStrictAggregateReturnType(expression, args.get(0)));
            return true;
        }

        if (isBuiltinFunctionName(functionName, "min")) {
            expression.setStaticSequenceType(
                    inferStrictMinMaxReturnType(expression, args.get(0), "fn:min"));
            return true;
        }

        if (isBuiltinFunctionName(functionName, "max")) {
            expression.setStaticSequenceType(
                    inferStrictMinMaxReturnType(expression, args.get(0), "fn:max"));
            return true;
        }
    ...
    @Override
    public StaticContext visitFunctionCall(
            FunctionCallExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        if (BuiltinFunctionCatalogue.exists(expression.getFunctionIdentifier(), queryLanguage)) {
            if (expression.isPartialApplication()) {
                // This should never be reached because partial application on built-in functions
                // should
                // have been
                // rewritten before
        ...
        try {
            signature =
                    getSignature(expression.getFunctionIdentifier(), expression.getStaticContext());
        } catch (UnknownFunctionCallException e) {
            throw new UnknownFunctionCallException(
                    expression.getFunctionIdentifier(), expression.getMetadata());
        }
        ...
        for (int i = 0; i < paramsLength; ++i) {
            if (parameterExpressions.get(i) != null) {
                ...
                if (actualType == null) {
                    throw new OurBadException(
                            "No static type inferred for expression "
                                    + parameterExpressions.get(i));
                }
                SequenceType expectedType = parameterTypes.get(i);
                // check actual parameters is either a subtype of or can be promoted to expected
                // type
        ...
        if (expression.isPartialApplication()) {
            FunctionSignature partialSignature =
                    new FunctionSignature(
                            partialParams, signature.getReturnType(), expression.isUpdating());
            expression.setStaticSequenceType(
            ...
            if (!tryAnnotateSpecificFunctions(expression, argument)) {
                ...
                if (BuiltinFunctionCatalogue.exists(
                                expression.getFunctionIdentifier(), queryLanguage)
    ...
    public StaticContext visitCastExpression(CastExpression expression, StaticContext argument) {
        ...
        SequenceType expressionSequenceType =
        ...
        if (expressionSequenceType.getArity() == SequenceType.Arity.One) {
            castedSequenceType =
                    new SequenceType(castedSequenceType.getItemType(), SequenceType.Arity.One);
        }
        expression.setStaticSequenceType(castedSequenceType);
        return argument;
    }
    ...
    @Override
    public StaticContext visitIsStaticallyExpr(
            IsStaticallyExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitDeleteExpression(
            DeleteExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitRenameExpression(
            RenameExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    @Override
    public StaticContext visitReplaceExpression(
            ReplaceExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    @Override
    public StaticContext visitInsertExpression(
            InsertExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    @Override
    public StaticContext visitAppendExpression(
            AppendExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    @Override
    public StaticContext visitEditCollectionExpression(
            EditCollectionExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        expression.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    public StaticContext visitAdditiveExpr(AdditiveExpression expression, StaticContext argument) {
        ...
        SequenceType.Arity inferredArity =
                resolveArities(leftInferredType.getArity(), rightInferredType.getArity());
    ...
    private ItemType resolveNumericType(ItemType left, ItemType right) {
        if (left.equals(BuiltinTypesCatalogue.doubleItem)
                || right.equals(BuiltinTypesCatalogue.doubleItem)) {
            return BuiltinTypesCatalogue.doubleItem;
        } else if (left.equals(BuiltinTypesCatalogue.floatItem)
                || right.equals(BuiltinTypesCatalogue.floatItem)) {
            return BuiltinTypesCatalogue.floatItem;
        } else if (left.equals(BuiltinTypesCatalogue.decimalItem)
    ...
    // For arithmetic operations, given 2 arities, return the resulting arity or null in case of
    // invalid arity
    ...
    private boolean checkSwitchType(
            List<Expression> expressions, SequenceType testType, ExceptionMetadata metadata) {
    ...
    @Override
    public StaticContext visitMultiplicativeExpr(
            MultiplicativeExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        if (leftItemType.isNumeric()) {
            if (rightItemType.isNumeric()) {
                if (expression.getMultiplicativeOperator()
                        == MultiplicativeExpression.MultiplicativeOperator.IDIV) {
                    ...
                    && expression.getMultiplicativeOperator()
                            == MultiplicativeExpression.MultiplicativeOperator.MUL) {
                inferredType = rightItemType;
            }
    ...
    public StaticContext visitUnaryExpr(UnaryExpression expression, StaticContext argument) {
        ...
        SequenceType childInferredType =
                requireInferredType(
                        expression.getMainExpression().getStaticSequenceType(), "UnaryExpression");
        ...
        // if inferred arity does not allow for empty sequence and static type is not an accepted
        // one
        // throw a static
        // error
        ...
        if (!childItemType.isNumeric()) {
            if (childInferredType.getArity() == SequenceType.Arity.OneOrZero) {
                ...
                throwStaticTypeException(
                        "It is not possible to have an Unary expression with the following type: "
                                + childInferredType,
                        expression.getMetadata());
            }
        }
        ...
        expression.setStaticSequenceType(
                new SequenceType(childItemType, childInferredType.getArity()));
        return argument;
    }
    ...
    private StaticContext visitAndOrExpr(
            Expression expression, StaticContext argument, String expressionName) {
        visitDescendants(expression, argument);

        List<Node> childrenExpressions = expression.getChildren();
        SequenceType leftInferredType =
                requireInferredType(
                        ((Expression) childrenExpressions.get(0)).getStaticSequenceType(),
                        expressionName + "Expression");
        SequenceType rightInferredType =
                requireInferredType(
    ...
    public StaticContext visitNotExpr(NotExpression expression, StaticContext argument) {
        ...
        SequenceType childInferredType =
                requireInferredType(
                        expression.getMainExpression().getStaticSequenceType(), "NotExpression");
    ...
    @Override
    public StaticContext visitComparisonExpr(
            ComparisonExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        // for value comparison arities * and + are not allowed, also if one return the empty
        // sequence
        // for sure throw
        // XPST0005 error
        if (operator.isValueComparison()) {
            ...
            if (returnArity == null) {
                throwStaticTypeException(
                        "'+' and '*' arities are not allowed for this comparison operator: "
                                + operator,
                        expression.getMetadata());
        ...
        // if any of the element is the empty sequence, we set its sequence type to the other, if
        // both
        // are we do not
        ...
        if (!isLeftEmpty || !isRightEmpty) {
            ...
            ItemType leftItemType =
                    isLeftEmpty ? rightInferredType.getItemType() : leftInferredType.getItemType();
            ItemType rightItemType =
                    isRightEmpty ? leftInferredType.getItemType() : rightInferredType.getItemType();
            ...
            // Type must match exactly or be both numeric or both promotable to string or both
            // durations
            // or one must be
            // null
            ...
            // Inequality is not defined for hexBinary and base64binary or for duration of different
            // types
        ...
        expression.setStaticSequenceType(
                new SequenceType(BuiltinTypesCatalogue.booleanItem, returnArity));
        return argument;
    }

    @Override
    public StaticContext visitNodeComparisonExpr(
            NodeComparisonExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        // According to XQuery 3.1 specification section 3.7.3:
        // Each operand must be either a single node or an empty sequence; otherwise a type error is
        // raised
        // [err:XPTY0004].
    ...
    public StaticContext visitNodeSetExpr(NodeSetExpression expression, StaticContext argument) {
        ...
        if (!leftType.isEmptySequence() && !leftType.getItemType().isNodeItemType()) {
            throwStaticTypeException(
                    "Left operand of a node set expression must be a sequence of nodes, got "
                            + leftType,
                    expression.getMetadata());
        }
        if (!rightType.isEmptySequence() && !rightType.getItemType().isNodeItemType()) {
            throwStaticTypeException(
                    "Right operand of a node set expression must be a sequence of nodes, got "
                            + rightType,
                    expression.getMetadata());
        }
    ...
    public StaticContext visitConditionalExpression(
            ConditionalExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        SequenceType resultingType =
                ifType.isSubtypeOf(SequenceType.createSequenceType("null?"))
                        ? elseType
    ...
    public void checkSwitchType(SequenceType type, ExceptionMetadata metadata) {
        if (type == null) {
            throw new OurBadException(
                    "A child expression of a SwitchExpression has no inferred type", metadata);
        }
        if (type.isEmptySequence()) {
            return; // no further check is required
        }
        if (type.getArity() == SequenceType.Arity.OneOrMore
                || type.getArity() == SequenceType.Arity.ZeroOrMore) {
            throwStaticTypeException(
    ...
    public StaticContext visitSwitchExpression(
            SwitchExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    public StaticContext visitTypeSwitchExpression(
        ...
        for (TypeswitchCase typeswitchCase : expression.getCases()) {
            ...
            // if we bind a variable we add the static type of it in the context of the return
            // expression
            if (variableName != null) {
                SequenceType variableType = null;
                for (SequenceType st : typeswitchCase.getUnion()) {
                    variableType =
                            variableType == null ? st : variableType.leastCommonSupertypeWith(st);
                }
                returnExpression
                        .getStaticContext()
                        .replaceVariableSequenceType(variableName, variableType);
            }
            ...
            inferredType =
                    inferredType == null
                            ? caseType
                            : inferredType.leastCommonSupertypeWith(caseType);
        }
        ...
        if (variableName != null) {
            returnExpression
                    .getStaticContext()
                    .replaceVariableSequenceType(variableName, conditionType);
        }
        visit(returnExpression, argument);
        ...
        inferredType =
                inferredType == null
                        ? defaultType
                        : inferredType.leastCommonSupertypeWith(defaultType);
    ...
    @Override
    public StaticContext visitStringConcatExpr(
            StringConcatExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitArrayLookupExpression(
            ArrayLookupExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        if (!lookupType.isSubtypeOf(SequenceType.createSequenceType("integer"))) {
            throwStaticTypeException(
                    "the lookup expression type must match integer, instead "
                            + lookupType
                            + " was inferred",
                    expression.getMetadata());
        }

        if (!mainType.hasOverlapWith(SequenceType.createSequenceType("array*"))
                || mainType.isEmptySequence()) {
            throwStaticTypeException(
                    "Inferred type is empty sequence and this is not a CommaExpression",
                    ErrorCode.StaticallyInferredEmptySequenceNotFromCommaExpression,
                    expression.getMetadata());
        ...
        SequenceType.Arity inferredArity =
                mainType.isAritySubtypeOf(SequenceType.Arity.OneOrZero)
                        ? SequenceType.Arity.OneOrZero
                        : SequenceType.Arity.ZeroOrMore;
    ...
    @Override
    public StaticContext visitObjectLookupExpression(
            ObjectLookupExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
        ...
        boolean overlapsObject =
                mainType.hasOverlapWith(SequenceType.createSequenceType("object*"));
        ...
        // if we have a specific object type and a string literal as key try to perform better
        // inference
        if (mainType.getItemType().isObjectItemType()
                && (expression.getLookupExpression()
                        instanceof StringLiteralExpression stringLiteralExpr)) {
            ...
            if (objectKeys.contains(key)) {
                ...
                if (field.isRequired()) {
                    // if the field is required then any object will have it, so no need to include
                    // '0' arity
                    // if not
                    // present
                ...
                // if object is closed and key is not found then for sure we will return the empty
                // sequence
                throwStaticTypeException(
                        "Inferred type is empty sequence and this is not a CommaExpression",
                        ErrorCode.StaticallyInferredEmptySequenceNotFromCommaExpression,
    ...
    @Override
    public StaticContext visitPostfixLookupExpression(
            PostfixLookupExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitUnaryLookupExpression(
            UnaryLookupExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitArrayUnboxingExpression(
            ArrayUnboxingExpression expression, StaticContext argument) {
        visitDescendants(expression, argument);
    ...
    @Override
    public StaticContext visitFilterExpression(
            FilterExpression expression, StaticContext argument) {
        ...
        predicateExpression
                .getStaticContext()
                .setContextItemStaticType(new SequenceType(mainType.getItemType()));
        ...
        if (!predicateType.hasEffectiveBooleanValue()) {
            throwStaticTypeException(
                    "Inferred type "
                            + predicateType
                            + " in FilterExpression has no effective boolean value",
                    expression.getMetadata());
        }

        // if we are filter one or less items or we use an integer to select a specific position we
        // return at most one
        // element, otherwise *
        SequenceType.Arity inferredArity =
                (mainType.isAritySubtypeOf(SequenceType.Arity.OneOrZero)
                                || predicateType
                                        .getItemType()
                                        .equals(BuiltinTypesCatalogue.integerItem))
                        ? SequenceType.Arity.OneOrZero
                        : SequenceType.Arity.ZeroOrMore;
    ...
    // return [true] if [types] are subtype of or can be promoted to expected types, [false]
    // otherwise
    ...
    public StaticContext visitDynamicFunctionCallExpression(
        ...
        if (isPartialApplication) {
            ...
            expression.setStaticSequenceType(
                    new SequenceType(ItemTypeFactory.createFunctionItemType(newSignature)));
            return argument;
        }
    ...
    @Override
    public StaticContext visitSimpleMapExpr(
            SimpleMapExpression expression, StaticContext argument) {
        ...
        rightExpression
                .getStaticContext()
                .setContextItemStaticType(new SequenceType(leftType.getItemType()));
    ...
    public StaticContext visitFlowrExpression(FlworExpression expression, StaticContext argument) {
        ...
        SequenceType.Arity forArities =
                SequenceType.Arity.One; // One is arity multiplication's neutral element
        ...
        while (clause != null) {
            ...
            // if there are for clauses we need to consider their arities for the returning
            // expression
            if (clause.getClauseType() == FLWOR_CLAUSES.FOR) {
                forType = ((ForClause) clause).getExpression().getStaticSequenceType();
                // if forType is the empty sequence that means that allowing empty is set otherwise
                // we would
                // have thrown
                // an error
                // therefore this for loop will generate one tuple binding the empty sequence, so as
                // for the
                // arities
                // count as arity.One
        ...
        SequenceType returnType =
                expression.getReturnClause().getReturnExpr().getStaticSequenceType();
        basicChecks(
                returnType,
                expression.getClass().getSimpleName(),
                true,
        ...
        returnType =
                new SequenceType(
                        returnType.getItemType(), returnType.getArity().multiplyWith(forArities));
        expression.setStaticSequenceType(returnType);
        return argument;
    }
    ...
    public StaticContext visitForClause(ForClause expression, StaticContext argument) {
        ...
        if (inferredType.isEmptySequence()) {
            ...
            // we take the single arity version of the inferred type or optional arity if we allow
            // empty
            // and the
            ...
            if (expression.isAllowEmpty()
                ...
                inferredType =
                        new SequenceType(inferredType.getItemType(), SequenceType.Arity.OneOrZero);
            } else {
    ...
    public StaticContext visitWindowClause(WindowClause expression, StaticContext argument) {
        ...
        SequenceType sourceType =
                expression.getActualSequenceType() == null
    ...
    private void checkWindowConditionType(Expression condition, WindowClause clause) {
        ...
        if (!conditionType.hasEffectiveBooleanValue()) {
            throwStaticTypeException(
                    "Window condition has no effective boolean value", clause.getMetadata());
        }
    }
    ...
    public StaticContext visitWhereClause(WhereClause expression, StaticContext argument) {
        ...
        if (whereType.isEmptySequence()
                || whereType.isSubtypeOf(SequenceType.createSequenceType("null?"))) {
            throwStaticTypeException(
    ...
    public StaticContext visitGroupByClause(GroupByClause expression, StaticContext argument) {
        Clause nextClause =
                expression
                        .getNextClause(); // != null because group by cannot be last clause of FLOWR
        // expression
        Set<Name> groupingVars = new HashSet<>();
        for (GroupByVariableDeclaration groupByVar : expression.getGroupVariables()) {
            // if we are grouping by an existing var (i.e. expr is null), then the appropriate type
            // is
            // already inferred
            ...
            if (groupByVarExpr != null) {
                ...
                expectedType =
                        expression
                                .getStaticContext()
                                .getVariableSequenceType(groupByVar.getVariableName());
            }
        ...
        // finally if there was a for clause we need to change the arity of the variables bound so
        // far
        // in the flwor
    ...
    // if [declaredType] is not null, check if the inferred type matches or can be promoted to the
    // declared type
    // (otherwise throw type error)
    // if [declaredType] is null, replace the type of [variableName] in the [context] with the
    // inferred type
    public void checkAndUpdateVariableStaticType(
        ...
        if (declaredType == null) {
            // if declared type is null, we overwrite the type in the correspondent InScopeVariable
            // with
            // the inferred
            // type
    ...
    public StaticContext visitVariableDeclaration(
            VariableDeclaration expression, StaticContext argument) {
        visitDescendants(expression, argument);
        SequenceType declaredType = expression.getActualSequenceType();
        SequenceType inferredType = SequenceType.createSequenceType("item*");
        if (declaredType == null) {
    ...
    public StaticContext visitFunctionDeclaration(
            FunctionDeclaration expression, StaticContext argument) {
        visitDescendants(expression, argument);
        InlineFunctionExpression inlineExpression =
                (InlineFunctionExpression) expression.getExpression();
        SequenceType inferredType =
                inlineExpression.getBody().getExpression().getStaticSequenceType();
    ...
    public StaticContext visitMainModule(MainModule mainModule, StaticContext argument) {
        StaticContext generatedContext =
    ...
    public StaticContext visitValidateTypeExpression(
        ...
        expression.setStaticSequenceType(
                expression.getSequenceType().refineArityIfSubtype(sourceType.getArity()));
        return argument;
    }
    ...
    private SequenceType getSequenceTypeFromChildren(
            SequenceType inferredType,
            SequenceType childSequenceType,
            ExceptionMetadata childMetadata) {
        // if a child expression has no inferred type throw an error
        if (childSequenceType == null) {
            throwStaticTypeException(
                    "A child expression of a BlockStatement has no inferred type", childMetadata);
        }
        ...
        if (childSequenceType != null && !childSequenceType.isEmptySequence()) {
            if (inferredType.isEmptySequence()) {
                ...
                ItemType resultingItemType =
                        inferredType
                                .getItemType()
                                .findLeastCommonSuperTypeWith(childSequenceType.getItemType());
                SequenceType.Arity resultingArity =
                        ((inferredType.getArity() == SequenceType.Arity.OneOrZero
                                                || inferredType.getArity()
                                                        == SequenceType.Arity.ZeroOrMore)
                                        && (childSequenceType.getArity()
                                                        == SequenceType.Arity.OneOrZero
                                                || childSequenceType.getArity()
                                                        == SequenceType.Arity.ZeroOrMore))
                                ? SequenceType.Arity.ZeroOrMore
    ...
    public StaticContext visitBlockStatement(BlockStatement statement, StaticContext argument) {
        ...
        for (Statement childStatement : statement.getBlockStatements()) {
            inferredType =
                    getSequenceTypeFromChildren(
                            inferredType,
                            childStatement.getStaticSequenceType(),
                            childStatement.getMetadata());
        }
    ...
    public StaticContext visitAssignStatement(AssignStatement statement, StaticContext argument) {
        ...
        SequenceType variableDeclaredType =
                statement.getStaticContext().getVariableSequenceType(statement.getName());
    ...
    @Override
    public StaticContext visitContinueStatement(
            ContinueStatement statement, StaticContext argument) {
        statement.setStaticSequenceType(SequenceType.createSequenceType("()"));
        return argument;
    }
    ...
    public StaticContext visitVariableDeclStatement(
            VariableDeclStatement statement, StaticContext argument) {
        visitDescendants(statement, argument);
    ...
    private static SequenceType getInferredSequenceType(
            VariableDeclStatement statement, SequenceType declaredType) {
        SequenceType inferredType = SequenceType.createSequenceType("item*");
        if (declaredType == null) {
    ...
    public StaticContext visitSlashExpr(SlashExpr slashExpr, StaticContext argument) {
        ...
        SequenceType leftType =
                requireInferredType(
                        slashExpr.getLeftExpression().getStaticSequenceType(), "SlashExpr");
        basicChecks(
                leftType,
                slashExpr.getClass().getSimpleName(),
                true,
                false,
                slashExpr.getMetadata());
        ...
        SequenceType rightType =
                requireInferredType(rightExpression.getStaticSequenceType(), "SlashExpr");
        basicChecks(
                rightType,
                slashExpr.getClass().getSimpleName(),
                true,
    ...
    public StaticContext visitStepExpr(StepExpr stepExpr, StaticContext argument) {
        ...
        if (contextType != null
            ...
            inferredType =
                    new SequenceType(inferredItemType, inferStepResultArity(stepExpr, contextType));
        }

        stepExpr.setStaticSequenceType(inferredType);
        basicChecks(
                inferredType,
                stepExpr.getClass().getSimpleName(),
                true,
                true,
    ...
    private ItemType inferStepResultItemType(StepExpr stepExpr) {
        ...
        if (nodeTest instanceof DocumentTest documentTest) {
            ...
            if (innerTest instanceof ElementTest elementTest
                    && elementTest.isNameWithoutTypeCheck()) {
        ...
        if (nodeTest instanceof NameTest nameTest) {
            boolean attributePrincipalKind =
    ...
    private boolean isStaticallyEmptyStep(StepExpr stepExpr, ItemType contextItemType) {
        if (stepExpr instanceof ForwardStepExpr forwardStep) {
            ...
            if (axis.equals(ForwardAxis.CHILD)
                ...
                boolean hasNoDescendants =
                        contextItemType.isSubtypeOf(BuiltinTypesCatalogue.attributeNode)
                                ...
                                || contextItemType.isSubtypeOf(
                                        BuiltinTypesCatalogue.processingInstructionNode);
                if (axis.equals(ForwardAxis.DESCENDANT_OR_SELF)) {
                    return hasNoDescendants
                            && !nodeTestCanMatchContextNode(
                                    stepExpr.getNodeTest(), contextItemType, axis);
                }
                return hasNoDescendants;
            }
            return false;
    ...
    private boolean nodeTestCanMatchContextNode(
            NodeTest nodeTest, ItemType contextItemType, ForwardAxis axis) {
        if (nodeTest instanceof AnyKindTest) {
            return true;
        }
        if (nodeTest instanceof TextTest) {

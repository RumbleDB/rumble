...
public class VariableValues implements Serializable {
    ...
    public boolean contains(Name varName) {
        boolean localContains =
                this.localVariableValues.containsKey(varName)
    ...
    public boolean isRDD(Name varName, ExceptionMetadata metadata) {
        if (!contains(varName)) {
            throw new OurBadException(
                    "Runtime error retrieving variable " + varName + " value.", metadata);
        }
    ...
    public List<Item> getLocalVariableValue(Name varName, ExceptionMetadata metadata) {
        if (this.localVariableValues.containsKey(varName)
                && this.localVariableValues.get(varName) == null) {
        ...
        if (this.dataFrameVariableValues.containsKey(varName)) {
            ...
            return HybridRuntimeIterator.collectRDDwithLimit(
                    df.toRDD(metadata), this.configuration, metadata);
        }
        ...
        if (this.localVariableCounts.containsKey(varName)) {
            throw new OurBadException(
                    "Runtime error retrieving variable "
                            + varName
                            + " value: only count available.",
                    metadata);
        }
    ...
    public HomogeneousItemDataFrame getDataFrameVariableValue(
            Name varName, ExceptionMetadata metadata) {
        if (this.dataFrameVariableValues.containsKey(varName)) {
            if (this.nestedQuery) {
                throw new JobWithinAJobException(metadata);
            }
    ...
    public String toString() {
        ...
        for (Name name : this.localVariableValues.keySet()) {
            sb.append(
                    "    " + name + " (" + this.localVariableValues.get(name).size() + " items)\n");
    ...
    private VariableValues findNodeWithVariableDeclaration(Name varName) {
        ...
        // Invariant: there is a node among the current node or its parents that contains the
        // variable

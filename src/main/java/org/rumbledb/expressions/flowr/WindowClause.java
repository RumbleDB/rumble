...
public class WindowClause extends Clause {
     ...
     *
     * <p>All of these variables are optional (can be null)
     *
     ...
     *
     * @param variables Variables bound to the condition (for example, {@code $start} in {@code
     *     start at $s when fn:true()}
     * @param expression boolean expression that triggers the start / end of window
     * @param only If a start item is identified, but no following item in the binding sequence
     *     satisfies the WindowEndCondition, then the only keyword determines whether a window is
     *     generated: if only end is specified, then no window is generated; otherwise, the end item
     *     is set to the last item in the binding sequence and a window is generated.
     */
    public record WindowCondition(WindowVars variables, Expression expression, boolean only) {}

    @Getter private final WindowType windowType;
    @Getter private final Name windowVariable;
    private final SequenceType sequenceType;
    @Getter private final Expression expression;
    @Getter private final WindowCondition startCondition;
    @Getter private final WindowCondition endCondition;

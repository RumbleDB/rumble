...
/** An XQuery validate expression, distinct from JSONiq sequence validation. */
@Getter
public class ValidateExpression extends Expression {
    ...
    public ValidateExpression(
            Expression mainExpression, ValidationMode validationMode, Name typeName, ExceptionMetadata metadata) {
        super(metadata);
        if (mainExpression == null || validationMode == null) {
            throw new OurBadException("A validate expression must have an operand and a validation mode.");
        }

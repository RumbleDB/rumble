...
public final class NamespaceBindingUtils {
    ...
    /**
     * Resolves a namespace prefix (including {@code ""} for the default element/type namespace) to
     * a URI, or {@code null} if unbound.
     */
    @FunctionalInterface
    ...
    public static NamespaceResolver namespaceResolver(RuntimeStaticContext runtimeStaticContext) {
        return runtimeStaticContext != null
                ? runtimeStaticContext::resolvePrefix
    ...
    /**
     * XQuery 3.1 computed constructors: dynamic error [err:XQDY0096] if the node-name has a
     * forbidden prefix/URI pair.
     */
    public static void validateConstructedNodeName(Name name, ExceptionMetadata metadata) {
        ReservedNamespaceBindingError error =
                getReservedNamespaceBindingError(name.getPrefix(), name.getNamespace());
        if (error == null) {
            return;
        }
        switch (error) {
            ...
            case XMLNS_PREFIX:
                throw new InvalidNodeNameException(
                        "The namespace prefix of the node-name is xmlns.", metadata);
    ...
    /**
     * XQuery 3.1 computed attribute constructors: if the attribute QName has a namespace URI but no
     * prefix, an implementation-defined prefix is used. The XML namespace is special-cased to use
     * the required {@code xml} prefix.
     */
    ...
    private static LexicalQNameSplit splitAndValidateLexicalQName(
            String lexical, ExceptionMetadata metadata) {
        if (lexical.isEmpty()) {
            throw new InvalidLexicalValueException(
                    "Invalid xs:QName: empty lexical value.", metadata);
        }
        ...
        if (colon < 0) {
            ...
            if (colon == 0
                    || colon == lexical.length() - 1
                    || lexical.indexOf(':', colon + 1) >= 0) {
                throw new InvalidLexicalValueException(
        ...
        if ("xmlns".equals(prefix)) {
            throw new InvalidLexicalValueException(
                    "Invalid xs:QName: prefix xmlns is not allowed.", metadata);
        }
    ...
    /**
     * fn:resolve-QName (Functions and Operators 3.1 10.1.1): like {@link #parseLexicalQName}, but
     * raises err:FONS0004 ({@link NoNamespaceFoundForPrefixException}) rather than err:FOCA0002
     * when the prefix is present but unbound.
     */
    ...
    /**
     * XQuery 3.1 computed attribute constructor: {@code xs:string} / {@code xs:untypedAtomic} name
     * is converted to an expanded QName. An unprefixed lexical form is a local name in <em>no</em>
     * namespace (not the default element/type namespace). A prefixed form resolves like {@link
     * #parseLexicalQName}.
     */
    public static Name parseLexicalQNameForComputedAttribute(
        ...
        if (lexical.startsWith("Q{")) {
            ...
            // XQuery EQNames use BracedURILiteral, which forbids brace characters inside the URI
            // part.
            // Dynamic computed attribute names coming from strings must reject malformed forms like
            // Q{{}x
            // or Q{}}x
            // during QName conversion so the constructor raises XQDY0074 rather than constructing a
            // bad
            // node-name.
            ...
            if (!isValidNcName(local)) {
                throw new InvalidLexicalValueException(
                        "Invalid URIQualifiedName local name: " + lexical, metadata);
            }
    ...
    /**
     * XPath and XQuery Functions 3.1 {@code fn:QName}: combines an optional namespace URI with a
     * lexical QName. Leading and trailing whitespace is stripped from both arguments (Functions and
     * Operators 3.1, section 10 intro).
     *
     * @param paramUriOrNull namespace URI, or {@code null} when {@code xs:string?} was the empty
     *     sequence
     * @param lexicalQName non-null lexical QName string (after atomization to {@code xs:string})
     */
    public static Name parseFnQName(
            String paramUriOrNull, String lexicalQName, ExceptionMetadata metadata) {
        if (lexicalQName == null) {
            throw new InvalidLexicalValueException(
                    "Invalid xs:QName: null lexical value.", metadata);
        }
    ...
    public static ReservedNamespaceBindingError getReservedNamespaceBindingError(
    ...
    public static void validateNamespaceDeclaration(String prefix, String uri) {
        ...
        // constructor.
        // Its purpose is
        // to bind a namespace prefix or to set the default element/type namespace for the
        // constructed
        // element node,
        // including its attributes.] Syntactically, a namespace declaration attribute has the form
        // of
        // an attribute
        // with namespace prefix xmlns, or with name xmlns and no namespace prefix."
        // "If the prefix of the attribute name is xmlns, then the local part of the attribute name
        // is
        // interpreted as a
        // namespace prefix."
        // "If the name of the namespace declaration attribute is xmlns with no prefix, then the
        // namespace URI specifies
        // the default element/type namespace of the constructor expression (overriding any existing
        // default), and is
        // added (with no prefix) to the in-scope namespaces of the constructed element (overriding
        // any
        // existing
    ...
    /**
     * Validates a namespace binding coming from parsed XML rather than from an XQuery namespace
     * declaration attribute. Unlike {@link #validateNamespaceDeclaration(String, String)}, this
     * permits prefixed empty URIs so that XML 1.1 undeclarations such as {@code xmlns:p=""} can be
    ...
    /**
     * Expanded name for an element or attribute DOM node (namespace-aware when the DOM provides
     * it).
     */
    ...
    /**
     * QName with only a local name and no namespace URI or prefix (XDM processing-instruction
     * target; namespace prefix property absent).
     */

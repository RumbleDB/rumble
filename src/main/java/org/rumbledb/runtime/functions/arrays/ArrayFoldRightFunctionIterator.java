...
public class ArrayFoldRightFunctionIterator extends ItemRuntimePlan implements LocalRuntimePlan<Item> {
    ...
    public ArrayFoldRightFunctionIterator(List<ItemRuntimePlan> arguments, RuntimeStaticContext staticContext) {
    ...
    private List<Item> computeResult(DynamicContext context) {
        ...
        try {
            ...
            throw new UnexpectedTypeException("array:fold-right expects exactly one array argument.", getMetadata());
        }
        if (arrayItem == null) {
        ...
        if (functionItems.isEmpty()) {
            throw new UnexpectedTypeException(
                    "Type error; third argument to array:fold-right must be a function item.", getMetadata());
        }
        if (functionItems.size() != 1 || !functionItems.get(0).isFunction()) {
            throw new UnexpectedTypeException(
                    "Type error; third argument to array:fold-right must be a single function item.", getMetadata());
        }

        FunctionItem functionItem = (FunctionItem) functionItems.get(0);

        for (int i = memberSequences.size() - 1; i >= 0; i--) {
            List<Item> memberSequence = memberSequences.get(i);
    ...
    private List<Item> applyFunction(
            FunctionItem functionItem, List<Item> memberSequence, List<Item> accumulator, DynamicContext context) {

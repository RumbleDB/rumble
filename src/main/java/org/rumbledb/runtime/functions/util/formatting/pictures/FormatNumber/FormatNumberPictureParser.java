...
public class FormatNumberPictureParser {
    ...
    public static FormatNumberPicture parse(
            String pictureString, DecimalFormatDefinition decimalFormat, ExceptionMetadata metadata) {
        ...
        if (separatorPos == -1) {
            FormatNumberSubPicture positiveSubpicture =
                    parseSubpicture(pictureString, pictureString, decimalFormat, metadata);
            FormatNumberSubPicture negativeSubpicture =
                    deriveNegativeSubpictureFromPositive(positiveSubpicture, decimalFormat);

            return new FormatNumberPicture(positiveSubpicture, negativeSubpicture);
        }
    ...
    private static FormatNumberSubPicture parseSubpicture(
        ...
        int leftmostActiveCharIndex =
                FormatNumberPictureSupport.findLeftmostActiveCharIndex(subPictureString, decimalFormat);
        int rightmostActiveCharIndex =
                FormatNumberPictureSupport.findRightmostActiveCharIndex(subPictureString, decimalFormat);
        ...
        if (hasExponent && !FormatNumberPictureSupport.containsOnlyMandatoryDigits(exponentPart, decimalFormat)) {
            throw invalidPicture(
                    pictureString, metadata, "exponent part must contain only decimal digit family characters");
        }

        // Find the decimal separator in the mantissa only.
        int decimalSeparatorIndex = FormatNumberPictureSupport.findDecimalSeparatorIndex(mantissaPart, decimalFormat);
        ...
        if (FormatNumberPictureSupport.countActiveDigitSigns(integerPart, decimalFormat)
        ...
        if (FormatNumberPictureSupport.hasAdjacentGroupingAndDecimalSeparator(activeRegion, decimalFormat)) {
            throw invalidPicture(
                    pictureString, metadata, "grouping separator must not be adjacent to decimal separator");
        }
        ...
        if (FormatNumberPictureSupport.hasAdjacentGroupingSeparators(activeRegion, decimalFormat)) {
            throw invalidPicture(pictureString, metadata, "subpicture must not contain adjacent grouping separators");
        }
        ...
        int percentOccurrences =
                FormatNumberPictureSupport.countOccurrences(subPictureString, decimalFormat.getPercent());
        int perMilleOccurrences =
                FormatNumberPictureSupport.countOccurrences(subPictureString, decimalFormat.getPerMille());
        ...
        if (hasPercent && hasPerMille) {
            throw invalidPicture(pictureString, metadata, "must not contain both percent sign and per-mille sign");
        }
        ...
        List<GroupingPos> integerPartGroupingPositions =
                FormatNumberPictureSupport.findIntegerGroupingPositions(integerPart, decimalFormat);

        Integer repeatingIntegerGroupingInterval = FormatNumberPictureSupport.findRepeatingIntegerGroupingInterval(
                integerPart, integerPartGroupingPositions, decimalFormat);

        // Analysis phase:
        // fractional-part-grouping-positions
        List<GroupingPos> fractionalPartGroupingPositions =
                FormatNumberPictureSupport.findFractionalGroupingPositions(fractionalPart, decimalFormat);
        ...
        if (FormatNumberPictureSupport.hasInvalidIntegerGroupingPosition(integerPartGroupingPositions)) {
            throw invalidPicture(
                    pictureString, metadata, "grouping separator must not appear at the end of the integer part");
        }

        if (FormatNumberPictureSupport.hasGroupingSeparatorAtEndOfFractionalPart(
                fractionalPart, fractionalPartGroupingPositions, decimalFormat)) {
            throw invalidPicture(
                    pictureString, metadata, "grouping separator must not appear at the end of the fractional part");
        }

        int minimumIntegerPartSize = FormatNumberPictureSupport.countMandatoryDigits(integerPart, decimalFormat);

        int scalingFactor = FormatNumberPictureSupport.countMandatoryDigits(integerPart, decimalFormat);

        int minimumFractionalPartSize = FormatNumberPictureSupport.countMandatoryDigits(fractionalPart, decimalFormat);

        int maximumFractionalPartSize = FormatNumberPictureSupport.countActiveDigitSigns(fractionalPart, decimalFormat);

        int minimumExponentSize = exponentPart.isEmpty()
                ? 0
    ...
    private static FormatNumberSubPicture deriveNegativeSubpictureFromPositive(
            FormatNumberSubPicture positiveSubpicture, DecimalFormatDefinition decimalFormat) {
    ...
    private static RumbleException invalidPicture(String pictureString, ExceptionMetadata metadata, String message) {
        return new IncorrectSyntaxFormatNumberException(
                "\"" + pictureString + "\": invalid picture string; " + message, metadata);
    }
}

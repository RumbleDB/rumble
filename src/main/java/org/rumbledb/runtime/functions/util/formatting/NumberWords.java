...
public final class NumberWords {
    // For plain ordinal word formatting (`Ww;o`) RumbleDB selects the ICU masculine ordinal rule
    // set
    // when available,
    // with a small set of aliases for ICU's abbreviated rule-set names. This is
    ...
    private static final Map<ULocale, LocalizedNumberFormatter> GROUPING_FORMATTER_CACHE =
    ...
    private static String formatWithRequestedRuleSet(
            long value, CachedRuleFormat f, String requestedRuleSet) {
    ...
    private static String ordinalWordWithSuffix(
            long value, CachedRuleFormat f, String requestedSuffix) {
    ...
    private static String requestedSuffix(String requestedRuleSet) {
        if (requestedRuleSet == null
                || requestedRuleSet.isEmpty()
                || requestedRuleSet.startsWith("%")) {
            return null;
        }

        String suffix =
                requestedRuleSet.startsWith("-") ? requestedRuleSet.substring(1) : requestedRuleSet;

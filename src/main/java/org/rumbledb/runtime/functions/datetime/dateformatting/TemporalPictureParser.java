...
final class TemporalPictureParser {
    ...
    static VariableMarker parse(String rawVariableMarker, String pictureString, ExceptionMetadata metadata) {
        String variableMarker = normalizeVariableMarker(rawVariableMarker);
        if (variableMarker.isEmpty()) {
            throw new IncorrectSyntaxFormatDateTimeException("\"" + pictureString + "\": incorrect syntax", metadata);
        }
        ...
        ParsedPresentationModifier presentation = ParsedPresentationModifier.parse(width.presentationPart);
        ...
        if (usesDefaultFallbackForSecondModifier(presentation.secondModifier)) {
            return defaultVariableMarker(component, width, ParsedPresentationModifier.NO_SECOND_MODIFIER);
        }
        ...
        if (component == 'f') {
            FractionalSecondsFormatter.validatePresentation(
                    presentation.firstPresentationModifier, pictureString, metadata);
        ...
        PrimaryFormatToken primaryFormatToken =
                parsePrimaryFormatToken(presentation.firstPresentationModifier, pictureString, metadata);
    ...
    static String normalizeVariableMarker(String s) {
        ...
        s.codePoints().filter(cp -> !Character.isWhitespace(cp)).forEach(sb::appendCodePoint);
        return sb.toString();
    }

    private static VariableMarker defaultVariableMarker(
            char component, WidthModifier width, char secondPresentationModifier) {
        if (component == 'f') {
            return VariableMarker.forFractionalSeconds(
        ...
        if (isDefaultNumericComponent(component)) {
            return VariableMarker.forNumeric(
                    component, "1", width.minWidth, width.maxWidth, secondPresentationModifier, null, false);
        }

        return VariableMarker.forDefault(component, "", width.minWidth, width.maxWidth, secondPresentationModifier);
    }

    private static PrimaryFormatToken parsePrimaryFormatToken(
            String presentation, String pictureString, ExceptionMetadata metadata) {
        try {
            return FormatIntegerPictureParser.parsePrimaryFormatToken(presentation, pictureString, metadata);
    ...
    private static final Set<Character> RECOGNIZED_COMPONENTS =
            Set.of('Y', 'M', 'D', 'd', 'F', 'W', 'w', 'H', 'h', 'P', 'm', 's', 'f', 'Z', 'z', 'C', 'E');

    private static final Set<Character> NAME_PRESENTABLE_COMPONENTS = Set.of('M', 'F', 'P');

    private static final Set<Character> FORMAT_INTEGER_PRESENTABLE_COMPONENTS =
            Set.of('Y', 'M', 'D', 'd', 'F', 'W', 'w', 'H', 'h', 'm', 's');

    private static final Set<Character> DEFAULT_NUMERIC_COMPONENTS =
            Set.of('Y', 'M', 'D', 'd', 'W', 'w', 'H', 'h', 'm', 's');

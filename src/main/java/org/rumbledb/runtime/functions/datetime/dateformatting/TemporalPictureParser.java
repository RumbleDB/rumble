...
final class TemporalPictureParser {
    ...
    static VariableMarker parse(
            String rawVariableMarker, String pictureString, ExceptionMetadata metadata) {
        ...
        ParsedPresentationModifier presentation =
                ParsedPresentationModifier.parse(width.presentationPart);

        if (component == 'Z' || component == 'z') {
            ParsedTimezonePicture tz =
                    TimezonePictureParser.parse(
                            component,
        ...
        if (usesDefaultFallbackForSecondModifier(presentation.secondModifier)) {
            return defaultVariableMarker(
                    component, width, ParsedPresentationModifier.NO_SECOND_MODIFIER);
        }
    ...
    private static VariableMarker defaultVariableMarker(
        ...
        if (component == 'F') {
            int maxWidth =
                    width.maxWidth == WidthModifier.UNBOUNDED && width.minWidth == 1
                            ? 3
                            : width.maxWidth;
            return VariableMarker.forName(
                    component, "", width.minWidth, maxWidth, secondPresentationModifier, null);
        }
    ...
    private static PrimaryFormatToken parsePrimaryFormatToken(
        ...
        try {
            return FormatIntegerPictureParser.parsePrimaryFormatToken(
                    presentation, pictureString, metadata);
    ...
    private static final Set<Character> RECOGNIZED_COMPONENTS =
            Set.of(
                    'Y', 'M', 'D', 'd', 'F', 'W', 'w', 'H', 'h', 'P', 'm', 's', 'f', 'Z', 'z', 'C',
                    'E');
        ...
        switch (presentation) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unsupported name presentation: " + presentation);
        }
    }
    ...
    private static String parseWordCase(String presentation) {
        switch (presentation) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unsupported word presentation: " + presentation);
        }
    }
}

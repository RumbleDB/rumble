...
final class TemporalComponentRenderer {
    ...
    static String render(
        ...
        switch (variableMarker.kind) {
            ...
            case VariableMarker.Kind.ALPHABETIC:
                return formatAlphabetic(
                        value, variableMarker, formattingContext, pictureString, metadata);

            case VariableMarker.Kind.NUMERIC:
                return formatNumericComponent(
                        value, variableMarker, formattingContext, pictureString, metadata);
    ...
    private static String formatRoman(
            ...
            ExceptionMetadata metadata) {
        int numericValue =
                variableMarker.numericValue(dt, formattingContext, pictureString, metadata);

        numericValue = applyYearMaximumWidthRule(numericValue, variableMarker);

        String roman =
                numericValue >= 1 && numericValue <= 3999
    ...
    private static String formatAlphabetic(
        ...
        String alpha =
                NumericFormattingSupport.integerToAlphabetic(
                        numericValue, variableMarker.lowerCaseAlphabetic);
    ...
    private static String formatNamedComponent(
        ...
        return TemporalFormattingSupport.applyNameCase(
                value, variableMarker, formattingContext.locale);
    }
    ...
    private static String formatWordsComponent(
        ...
        String words =
                variableMarker.isOrdinal()
                        ? NumberWords.ordinalWords(
                                numericValue,
                                formattingContext.uLocale,
                                variableMarker.formatSpecifier)
                        : NumberWords.cardinal(
                                numericValue,
                                formattingContext.uLocale,
                                variableMarker.formatSpecifier);

        return TemporalFormattingSupport.applyWordCase(
                words, variableMarker.wordCase, formattingContext.locale);
    }
    ...
    private static String maybeAppendOrdinal(
        ...
        return base
                + NumberWords.ordinalSuffix(
                        BigInteger.valueOf(numericValue), formattingContext.uLocale);
    }
    ...
    private static int applyYearMaximumWidthRule(int value, VariableMarker variableMarker) {
        if (variableMarker.appliesYearMaximumWidthRule() && variableMarker.maxWidth > 0) {
            ...
            if (digits.length() > variableMarker.maxWidth) {
                int reduced =
                        Integer.parseInt(
                                digits.substring(digits.length() - variableMarker.maxWidth));

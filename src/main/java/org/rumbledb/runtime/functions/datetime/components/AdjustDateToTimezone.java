    ...
    public Item materializeFirstItemOrNull(DynamicContext context) {
        ...
        if (this.timezone == null && this.getChildren().size() == 1) {
            return ItemFactory.getInstance()
                    .createDateItem(
                            dateItem.getDateTimeValue().withOffsetSameInstant(ZoneOffset.UTC),
                            true);
        }
        if (this.timezone == null) {
            return ItemFactory.getInstance()
                    .createDateItem(
                            dateItem.getDateTimeValue().withOffsetSameLocal(ZoneOffset.UTC), false);
        } else {
            ...
            return ItemFactory.getInstance()
                    .createDateItem(
                            dateItem.getDateTimeValue()
                                    .withOffsetSameLocal(ZoneOffset.ofHoursMinutes(hours, minutes)),
                            true);
        }
    }

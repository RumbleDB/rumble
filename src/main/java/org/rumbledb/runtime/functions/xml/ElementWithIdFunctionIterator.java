...
public class ElementWithIdFunctionIterator extends HybridRuntimeIterator {
    ...
    private void computeResults() {
        ...
        if (node == null || !node.isNode()) {
            throw new UnexpectedTypeException(
                    "The argument to fn:element-with-id must be a node", getMetadata());
        }
        Item root = node;
    ...
    protected Item nextLocal() {
        if (!this.hasNext) {
            throw new IteratorFlowException(
                    RuntimeIterator.FLOW_EXCEPTION_MESSAGE + " fn:element-with-id", getMetadata());
        }
    ...
    public JavaRDD<Item> getRDDAux(DynamicContext context) {
        throw new OurBadException(
                "fn:element-with-id is currently supported only in local execution mode.");
    }

...
/**
 * Implementation of the fn:nilled function according to XPath and XQuery Functions and Operators
 * 3.1 ({@code https://www.w3.org/TR/xpath-functions-31/#func-nilled}) and the XDM 3.1 nilled
 * accessor.
 *
 * <p>XDM 3.1 Section 5.8 nilled Accessor.
 *
 * <p>dm:nilled($n as node()) as xs:boolean?
 *
 * <p>"The dm:nilled accessor returns true if the element node is nilled, false if the element node
 * is not nilled, or the empty sequence if the concept of nilled does not apply."
 *
 * <p>In this API, the optional xs:boolean result is represented as a sequence of zero or one Items
 * returned by Item.nilled().
 *
 * <p>Function signature (Functions and Operators 3.1, {@code fn:nilled}):
 *
 * <ul>
 *   <li>fn:nilled($arg as node()?) as xs:boolean?
 * </ul>
 *
 * Rules:
 *
 ...
 *       sequence.
 *   <li>Otherwise, the function returns dm:nilled($arg).
 * </ul>
 *
 * @see <a href="https://www.w3.org/TR/xpath-functions-31/#func-nilled">XPath and XQuery Functions
 *     and Operators 3.1: fn:nilled</a>
 */
    ...
    public Item next() {
        if (!this.hasNext) {
            throw new IteratorFlowException(
                    RuntimeIterator.FLOW_EXCEPTION_MESSAGE + " nilled function", getMetadata());
        }

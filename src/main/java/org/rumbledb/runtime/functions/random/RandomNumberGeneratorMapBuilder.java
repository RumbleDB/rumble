...
/**
 * Builds the three-entry map (number/next/permute) returned by fn:random-number-generator, per
 * F&amp;O 3.1 4.9.1. Given the same seed, always builds an equal map (number and the seed chain
 * reachable via next() are a pure function of the seed).
 */
final class RandomNumberGeneratorMapBuilder {

    static final Name PERMUTE_PARAM_NAME =

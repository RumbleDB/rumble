...
/**
 * Function conversion step for arguments: cast runtime xs:untypedAtomic values to the requested
 * atomic type before the generic type-promotion layer runs. Non-untyped values flow through
 * unchanged.
 */
    ...
    private Item castIfUntypedAtomic(Item item) {
        ...
        Item converted =
                CastIterator.castItemToType(
                        item, this.targetType, getMetadata(), this.staticContext);
        if (converted == null) {
            throw new UnexpectedTypeException(
                    this.exceptionMessage
                            + item.getDynamicType()
        ...
        return itemType.equals(BuiltinTypesCatalogue.QNameItem)
                || (itemType.isAtomicItemType()
                        ...
                        && itemType.getCastingPrimitiveType()
                                .equals(BuiltinTypesCatalogue.QNameItem));
    }
    ...
    private boolean isNotationLikeTargetType(ItemType itemType) {
        return itemType.equals(BuiltinTypesCatalogue.NOTATIONItem)
                || (itemType.isAtomicItemType()
                        && !itemType.equals(BuiltinTypesCatalogue.errorItem)
                        && itemType.getCastingPrimitiveType()
                                .equals(BuiltinTypesCatalogue.NOTATIONItem));
    }

...
public class FunctionCoercionRuntimeIterator extends HybridRuntimeIterator {
    ...
    @Getter private final Item callableItem;
    ...
    private RuntimeIterator buildDelegate(DynamicContext context) {
        ...
        RuntimeStaticContext callStaticContext =
        ...
        if (this.callableItem.isArray()) {
            return new ArrayFunctionCallIterator(
                    this.callableItem, arguments.get(0), callStaticContext);
        }
        if (this.callableItem.isMap()) {
            return new MapFunctionCallIterator(
                    this.callableItem, arguments.get(0), callStaticContext);
        }
        if (!this.callableItem.isFunction()) {
            throw new OurBadException(
                    "Function coercion can only wrap functions, maps, or arrays.", getMetadata());
        }
        RuntimeIterator callIterator =
    ...
    private RuntimeIterator buildArgumentIterator(Name parameterName, DynamicContext context) {
        ...
        RuntimeStaticContext parameterStaticContext =
                getRuntimeStaticContext().toBuilder()
                        .staticType(SequenceType.createSequenceType("item*"))

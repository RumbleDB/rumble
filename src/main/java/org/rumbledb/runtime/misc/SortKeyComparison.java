...
/**
 * Lexicographic comparison of sort-key sequences for F&amp;O 3.1 {@code fn:sort} / {@code
 * array:sort} ({@code deep-less-than} and {@code fn:deep-equal} on key sequences with a collation).
 */
public final class SortKeyComparison {
    ...
    public static boolean sortKeysDeepLessThan(
        ...
        if (cmp == Long.MIN_VALUE) {
            throw new UnexpectedTypeException(
                    "Sort keys contain values that are not comparable.", metadata);
        }
        return cmp < 0;
    }
    ...
    private static boolean sortKeyItemDeepEqual(
        ...
        DeepEqualFunctionIterator deepEqual =
                new DeepEqualFunctionIterator(Collections.emptyList(), staticContext);
        return deepEqual.checkDeepEqual(Collections.singletonList(a), Collections.singletonList(b));
    }

    private static int compareStringCollationTypes(
            Item a, Item b, String collationUri, ExceptionMetadata metadata) {

...
//    }
    ...
    public static PendingUpdateList mergeUpdates1(PendingUpdateList pul1, PendingUpdateList pul2) {
        PendingUpdateList result = new PendingUpdateList();
        HashMap<UpdatePrimitiveTarget, HashMap<UpdatePrimitiveEnum, Set<UpdatePrimitiveSource>>> itemToUpdateMap = new HashMap<>();
        HashMap<UpdatePrimitiveEnum, Set<UpdatePrimitiveSource>> temp;
        Set<UpdatePrimitiveSource> tempDistinctSources;
        ...
        for (List<UpdatePrimitive> us : Arrays.asList(pul1.getUpdatePrimitives(), pul2.getUpdatePrimitives())) {
            for (UpdatePrimitive u : us) {
                ...
                temp = itemToUpdateMap.getOrDefault(target, new HashMap<>());

                if (u instanceof DeleteFromArrayPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.DELETE_FROM_ARRAY, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.DELETE_FROM_ARRAY, tempDistinctSources);
                } else if (u instanceof InsertIntoArrayPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.INSERT_INTO_ARRAY, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.INSERT_INTO_ARRAY, tempDistinctSources);
                } else if (u instanceof ReplaceInArrayPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.REPLACE_IN_ARRAY, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.REPLACE_IN_ARRAY, tempDistinctSources);
                } else if (u instanceof DeleteFromObjectPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.DELETE_FROM_OBJECT, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.DELETE_FROM_OBJECT, tempDistinctSources);
                } else if (u instanceof InsertIntoObjectPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.INSERT_INTO_OBJECT, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.INSERT_INTO_OBJECT, tempDistinctSources);
                } else if (u instanceof ReplaceInObjectPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.REPLACE_IN_OBJECT, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.REPLACE_IN_OBJECT, tempDistinctSources);
                } else if (u instanceof RenameInObjectPrimitive) {
                    tempDistinctSources = temp.getOrDefault(UpdatePrimitiveEnum.RENAME_IN_OBJECT, new HashSet<>());
                    tempDistinctSources.add(u.getSource());
                    temp.put(UpdatePrimitiveEnum.RENAME_IN_OBJECT, tempDistinctSources);
                }

                itemToUpdateMap.put(target, temp);
            }
        }

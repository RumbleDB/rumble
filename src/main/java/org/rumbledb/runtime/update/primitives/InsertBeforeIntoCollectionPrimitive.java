...
public class InsertBeforeIntoCollectionPrimitive implements UpdatePrimitive {
    ...
    public InsertBeforeIntoCollectionPrimitive(Item target, Dataset<Row> contents, ExceptionMetadata metadata) {
        this.target = target;
    ...
    public void applyDelta() {
        ...
        Dataset<Row> rowNumOrderDF = rowNumDF3
                .withColumn(
                        SparkSessionManager.rowOrderColumnName,
                        expr(String.format("%f + (%s) * %f", rowOrderBase, tmpRowNumOrder, interval))
                                .cast("double"))
                .drop(tmpRowNumOrder);

...
public class InsertBeforeIntoCollectionPrimitive implements UpdatePrimitive {
    ...
    public void applyDelta() {
        ...
        Row aggRow =
                session.table(this.collection.getPhysicalName())
        ...
        Dataset<Row> rowNumDF2 =
                rowNumDF.withColumn(tmpRowNumSeq, row_number().over(Window.orderBy(tmpRowNum)))
                        .drop(tmpRowNum);
        Dataset<Row> rowIdDF =
                rowNumDF2
                        .withColumn(
                                ...
                                expr(
                                        String.format(
                                                "cast(%d as long) + %s", rowIDStart, tmpRowNumSeq)))
                        .drop(tmpRowNumSeq);

        Dataset<Row> rowNumDF3 =
                rowIdDF.withColumn(tmpRowNumOrder, row_number().over(Window.orderBy(lit(1))));
        Dataset<Row> rowNumOrderDF =
                rowNumDF3
                        .withColumn(
                                SparkSessionManager.rowOrderColumnName,
                                expr(String.format(
                                                "%f + (%s) * %f",
                                                rowOrderBase, tmpRowNumOrder, interval))
                                        .cast("double"))
                        .drop(tmpRowNumOrder);

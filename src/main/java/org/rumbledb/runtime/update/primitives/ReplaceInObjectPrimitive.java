        ...
        if (startOfArrayIndexing == -1) {
            ...
            if (!fullFieldPath.contains(".")) {
                ...
                boolean replacementIsComplex =
                        replacementTypeUpper.startsWith("STRUCT<")
                                || replacementTypeUpper.startsWith("ARRAY<");
                boolean currentIsComplex =
                        currentTypeUpper.startsWith("STRUCT<")
                                || currentTypeUpper.startsWith("ARRAY<");
                if (replacementIsComplex && !currentIsComplex) {
                    // Spark cannot cast primitive -> struct/array. Drop/add the column to evolve
                    // schema.
                    String[] fieldNames =
                            SparkSessionManager.getInstance()
                                    .getOrCreateSession()
            ...
            this.applySetFieldInCollection(
                    location, rowID, fullFieldPath, this.content.getSparkSQLValue());
        } else {

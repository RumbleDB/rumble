    ...
    public PendingUpdateList getPendingUpdateList(DynamicContext context) {
        ...
        try {
            ...
            throw new InvalidUpdateTargetException(
                    "More than one target item cannot be Edited.", this.getMetadata());
        } catch (NoItemException e) {
            throw new InvalidUpdateTargetException(
                    "One target item must be provided for Edit.", this.getMetadata());
        }
        ...
        if (contentCount != 1) {
            throw new InvalidUpdateTargetException(
                    "Exactly one content must be specified for edit, but "
                            + contentCount
                            + " found",
                    this.getMetadata());
        }
        ...
        UpdatePrimitive up =
                factory.createEditTuplePrimitive(targetItem, contentDF, this.getMetadata());

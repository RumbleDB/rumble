...
public class TypePromotionIterator extends ItemRuntimePlan
    ...
    private static void checkEmptySequence(
            long size, SequenceType sequenceType, String exceptionMessage, ExceptionMetadata metadata) {
        if (size == 0 && sequenceType.getArity() == SequenceType.Arity.OneOrMore) {
            throw new UnexpectedTypeException(
                    exceptionMessage + "Expecting at least one item, but the value provided is the empty sequence.",
                    metadata);
        }
    }
    ...
    public HomogeneousItemDataFrame createNativeDataFrame(DynamicContext dynamicContext) {
        ...
        checkEmptySequence(df.isEmptySequence() ? 0 : 1, this.sequenceType, this.exceptionMessage, getMetadata());
        ...
        throw new UnexpectedTypeException(
                this.exceptionMessage + dataItemType + " cannot be promoted to type " + this.sequenceType + ".",
                getMetadata());
    }
    ...
    private static Item promoteItem(
        ...
        if ((item.isFunction() || item.isMap() || item.isArray())
            ...
            return FunctionCoercion.coerceToFunctionItem(item, itemType, staticContext, exceptionMessage);
        }
        ...
        Item promotedItem = CastIterator.castItemToType(item, sequenceType.getItemType(), metadata, staticContext);
        if (promotedItem == null) {
            throw new OurBadException("We were not able to promote " + item + " to type " + sequenceType.getItemType());
        }
        return promotedItem;
    }
    ...
    private static final class EvaluationCursor extends AbstractLocalCursor<Item> {
        ...
        private void setNextResult() {
            ...
            if (!this.childCursor.hasNext()) {
                checkEmptySequence(this.childIndex, this.sequenceType, this.exceptionMessage, this.metadata);
                return;
            }

...
public final class TypeInferrenceUtils {
    ...
    public static ItemType inferItemTypeOfRDDItems(
        ...
        return itemRDD.aggregate(
                ...
                (ItemType acc, Item item) -> {
                    ...
                    return acc.equals(neutralElement)
                            ? itemType
                            : mergeItemTypes(acc, itemType, typeMergeMode);
                },
    ...
    public static ItemType inferItemTypeOfLocalItems(
        ...
        for (int i = 1; i < items.size(); i++) {
            result =
                    mergeItemTypes(
                            result,
                            ItemTypeFactory.createItemTypeFromItem(items.get(i)),
                            typeMergeMode);
        }
    ...
    public static SequenceType inferSequenceTypeOfLocalItems(
            List<Item> items, TypeMergeMode typeMergeMode) {
        if (items.isEmpty()) {
            return SequenceType.createSequenceType("()");
        }
        ItemType itemType =
                inferItemTypeOfLocalItems(items, ExceptionMetadata.EMPTY_METADATA, typeMergeMode);
        if (items.size() == 1) {
    ...
    private static ItemType mergeItemTypes(
            ItemType left, ItemType right, TypeMergeMode typeMergeMode) {
    ...
    private static SequenceType mergeSequenceTypes(
            SequenceType left, SequenceType right, TypeMergeMode typeMergeMode) {
        ...
        ItemType itemSupertype =
                mergeItemTypes(left.getItemType(), right.getItemType(), typeMergeMode);

...
public class SimpleMapExpressionIterator extends ItemRuntimePlan
    ...
    public SimpleMapExpressionIterator(
            ItemRuntimePlan sequence, ItemRuntimePlan mapExpression, RuntimeStaticContext staticContext) {
    ...
    private static final class SimpleMapLocalCursor extends AbstractLocalCursor<Item> {
        ...
        protected boolean hasNextLocal() {
            while (this.currentResults == null || !this.currentResults.hasNext()) {
                ...
                mapContext
                        .getVariableValues()
                        .addVariableValue(Name.CONTEXT_ITEM, List.of(this.inputs.get(this.inputIndex)));
        ...
        protected Item nextLocal() {
            if (!hasNextLocal()) {
                throw new IteratorFlowException("Invalid next() call in simple map expression", this.metadata);
            }
    ...
    public HomogeneousItemDataFrame createNativeDataFrame(DynamicContext context) {
        HomogeneousItemDataFrame df = ItemRuntimeDataFrameFactory.INSTANCE.fromPlan(this.leftIterator, context);
        if (df.isEmptySequence()) {
            return df;
        }
        NativeClauseContext forContext =
                new NativeClauseContext(FLWOR_CLAUSES.FOR, df.getDataFrame().schema(), context);
        NativeClauseContext nativeQuery = NativeQueryRuntimePlan.generate(this.rightIterator, forContext);
        ...
        Dataset<Row> result = df.getDataFrame()
                ...
                .sql(String.format(
                        ...
                        nativeQuery.getResultingQuery(), SparkSessionManager.nonObjectJSONiqItemColumnName, input));
        // execute query
        return new HomogeneousItemDataFrame(result, getStaticType().getItemType());
    }
}

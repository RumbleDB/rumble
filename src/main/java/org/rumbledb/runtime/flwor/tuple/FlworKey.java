...
public class FlworKey {
     ...
     * @param flworKey "other" FlworKey to be compared against
     * @param metadata
     * @return comparison value (-1=smaller, 0=equal, 1=larger) * index (of the expression that
     *     determines ordering)
     */
            ...
            switch (emptyOrder) {
                case LEAST:
                    ...
                    comparison =
                            ComparisonIterator.compareItems(
                                    item1, item2, ComparisonOperator.VC_EQ, metadata);
                ...
                case NONE:
                    throw new OurBadException(
                            "Behavior of empty sequence ordering was not resolved", metadata);
            }

            // Simplify comparison result to -1/0/1
            result = (int) Math.signum(result);

            // if comparison result is not an equality, return it multiplied with the index of the
            // expression compared
            if (result != 0) {
                return result
                        * (index + 1); // use index+1 to prevent multiplication w/ 0 for the first
                // index
            }
            index++;
        }

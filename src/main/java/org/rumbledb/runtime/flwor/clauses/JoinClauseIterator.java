...
public class JoinClauseIterator extends RuntimeTupleIterator {
     ...
     *
     * <p>Warning: if the two tuples collide in their columns, unexpected things may happen for now.
     *
     ...
     * @param outputTupleVariableDependencies the necessary and sufficient variable dependencies
     *     that the output tuple should contain.
     ...
     * @param newRightSideVariableName the new name of the variable to rename the context item in
     *     the output (null if no rename).
    ...
    public static FlworDataFrame joinInputTupleWithSequenceOnPredicate(
        ...
        boolean optimizableJoin =
                extractEqualityComparisonsForHashing(
                        predicateIterator,
        ...
        if (newRightSideVariableName != null
            ...
            predicateDependencies.put(
                    Name.CONTEXT_ITEM,
                    outputTupleVariableDependencies.get(newRightSideVariableName));
        }
        ...
        if (rightTupleSideEqualityCriteria.size() == 1) {
            ...
            RuntimeStaticContext rhsStaticContext =
            ...
            rightHandSideEqualityCriterion =
                    new ArrayRuntimeIterator(
                            new CommaExpressionIterator(
                                    rightTupleSideEqualityCriteria, rhsStaticContext),
                            rhsStaticContext,
                            false);
        }
        if (leftTupleSideEqualityCriteria.size() == 1) {
            ...
            RuntimeStaticContext lhsStaticContext =
                    staticContext.toBuilder()
                            .staticType(SequenceType.createSequenceType("item*"))
                            .executionMode(ExecutionMode.LOCAL)
                            .metadata(metadata)
            ...
            leftHandSideEqualityCriterion =
                    new ArrayRuntimeIterator(
                            new CommaExpressionIterator(
                                    leftTupleSideEqualityCriteria, lhsStaticContext),
                            lhsStaticContext,
                            false);
        }
        ...
        String projectionVariables =
                FlworDataFrameUtils.getSQLColumnProjection(
                        columnsToSelect, newRightSideVariableName != null);
        if (newRightSideVariableName != null) {
            projectionVariables +=
                    String.format(
                            " `%s`.`%s` AS `%s`",
        ...
        List<FlworDataFrameColumn> joinCriterionUDFcolumns =
                FlworDataFrameUtils.getColumns(
                        jointSchema,
                        predicateIterator.getVariableDependencies(),
        ...
        String UDFParameters =
                FlworDataFrameUtils.getUDFParametersFromColumns(joinCriterionUDFcolumns);
        ...
        while (!candidateIterators.isEmpty()) {
            ...
            if (iterator instanceof AndOperationIterator andOperationIterator) {
                ...
                if (comparisonIterator.isValueEquality()) {
                    ...
                    Set<Name> leftComparisonDependencies =
                            new HashSet<>(lhs.getVariableDependencies().keySet());
                    Set<Name> rightComparisonDependencies =
                            new HashSet<>(rhs.getVariableDependencies().keySet());
                    // TODO it would be nice to be more generic and also allow dependencies on the
                    // dynamic context on any side.
                    if (leftTupleSideVariableNames.containsAll(leftComparisonDependencies)
                            && rightTupleSideVariableNames.containsAll(
                                    rightComparisonDependencies)) {
                        optimizableJoin = true;
                        leftTupleSideEqualityCriteria.add(lhs);
                        rightTupleSideEqualityCriteria.add(rhs);
                    }
                    if (leftTupleSideVariableNames.containsAll(rightComparisonDependencies)
                            && rightTupleSideVariableNames.containsAll(
                                    leftComparisonDependencies)) {
                        optimizableJoin = true;
    ...
    /**
     * Try to generate the native query for the group by clause and run it, if successful return the
     * resulting dataframe, otherwise it returns null (expect `input` table to be already available)
     *
    ...
    private static FlworDataFrame tryNativeQueryStatically(
        ...
        NativeClauseContext nativeContext =
                new NativeClauseContext(FLWOR_CLAUSES.WHERE, unionSchema, context);
        ...
        List<FlworDataFrameColumn> columnsToSelect =
                FlworDataFrameUtils.getColumns(
                        ...
                        Collections.emptyList());
        String projectionVariables =
                FlworDataFrameUtils.getSQLColumnProjection(columnsToSelect, false);
        Dataset<Row> result =
                leftInputTuple
                        .sparkSession()
                        .sql(

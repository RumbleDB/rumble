...
public class OrderClauseCreateColumnsUDF implements UDF1<Row, Row> {
    ...
    private static final int emptySequenceOrderIndexFirst =
            1; // by default, empty sequence is taken as first(=least)
    private static final int emptySequenceOrderIndexLast =
            4; // by default, empty sequence is taken as first(=least)
    private static final int nullOrderIndex =
            2; // null is the smallest value except empty sequence(default)
    private static final int valueOrderIndex =
            3; // values are larger than null and empty sequence(default)
    ...
    public Row call(Row row) {
        ...
        for (int expressionIndex = 0;
                expressionIndex < this.expressionsWithIterator.size();
                expressionIndex++) {
            ...
            if (!iterator.hasNext()) {
                if (expressionWithIterator.getEmptyOrder()
                        == OrderByClauseSortingKey.EMPTY_ORDER.GREATEST) {

...
public class AdditiveOperationIterator extends AtMostOneItemLocalRuntimeIterator {
	...
	public AdditiveOperationIterator(RuntimeIterator leftIterator, RuntimeIterator rightIterator, boolean isMinus,
	...
	public Item materializeFirstItemOrNull(DynamicContext dynamicContext) {
		...
		if (result == null) {
			throw new UnexpectedTypeException(
					" \"+\": operation not possible with parameters of type \"" + this.left.getDynamicType().toString()
							+ "\" and \"" + this.right.getDynamicType().toString() + "\"",
					getMetadata());
		}
		return result;
	}
	...
	public static Item processItem(Item left, Item right, boolean isMinus) {
		...
		if (left.isInt() && right.isInt()) {
			if (right.isInt() && (left.getIntValue() < Integer.MAX_VALUE / 2
					&& left.getIntValue() > -Integer.MAX_VALUE / 2 && right.getIntValue() > -Integer.MAX_VALUE / 2
	...
	public NativeClauseContext generateNativeQuery(NativeClauseContext nativeClauseContext) {
		...
		} else if (this.rightIterator.getStaticType().isSubtypeOf(SequenceType.DOUBLE_QM)
				&& this.leftIterator.getStaticType().getItemType().isNumeric()) {
			if (!this.leftIterator.getStaticType().isSubtypeOf(SequenceType.DOUBLE_QM)) {
				leftQuery = "(CAST (" + leftQuery + " AS DOUBLE))";
			}
		} else if (this.leftIterator.getStaticType().isSubtypeOf(SequenceType.FLOAT_QM)
				&& this.rightIterator.getStaticType().getItemType().isNumeric()) {
			if (!this.rightIterator.getStaticType().isSubtypeOf(SequenceType.FLOAT_QM)) {
				rightQuery = "(CAST (" + rightQuery + " AS FLOAT))";
			}
		} else if (this.rightIterator.getStaticType().isSubtypeOf(SequenceType.FLOAT_QM)
				&& this.leftIterator.getStaticType().getItemType().isNumeric()) {
			if (!this.leftIterator.getStaticType().isSubtypeOf(SequenceType.FLOAT_QM)) {
				leftQuery = "(CAST (" + leftQuery + " AS FLOAT))";
			}
		} else if (this.leftIterator.getStaticType().isSubtypeOf(SequenceType.INTEGER_QM)
				&& this.rightIterator.getStaticType().isSubtypeOf(SequenceType.INTEGER_QM)) {
		} else if (this.leftIterator.getStaticType().isSubtypeOf(SequenceType.DECIMAL_QM)

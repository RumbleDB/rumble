...
public class ArrayUnboxingIterator extends HybridRuntimeIterator {
    ...
    public ArrayUnboxingIterator(
            RuntimeIterator arrayIterator, RuntimeStaticContext staticContext) {
    ...
    public NativeClauseContext generateNativeQuery(NativeClauseContext nativeClauseContext) {
        ...
        newContext.setResultingType(
                new SequenceType(
                        newContextType.getArrayContentFacet(), SequenceType.Arity.ZeroOrMore));
        ...
        if (lateralViewPart.size() == 0) {
            ...
            // if we have multiple array unboxing we stack multiple lateral views and each one takes
            // from
            // the
            // previous
            lateralViewPart.add(
                    "explode( arr"
                            + lateralViewPart.size()
                            + ".col"
                            + newContext.getResultingQuery()
                            + ")");
        }
    ...
    public NativeClauseContext generateArrayReferenceQuery(

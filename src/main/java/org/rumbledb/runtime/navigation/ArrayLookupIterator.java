...
public class ArrayLookupIterator extends HybridRuntimeIterator {
    ...
    public ArrayLookupIterator(
            RuntimeIterator array, RuntimeIterator iterator, RuntimeStaticContext staticContext) {
    ...
    public NativeClauseContext generateNativeQuery(NativeClauseContext nativeClauseContext) {
        ...
        if (newContext != NativeClauseContext.NoNativeQuery) {
            if (SequenceType.Arity.OneOrMore.isSubtypeOf(
                    newContext.getResultingType().getArity())) {
            ...
            if (schema instanceof StructType structType) {
                ...
                if (Arrays.stream(structSchema.fieldNames())
                        .anyMatch(
                                field ->
                                        keyDependencies.containsKey(
                                                Name.createVariableInNoNamespace(field)))) {
                    return NativeClauseContext.NoNativeQuery;
                }
            }
            ...
            newContext.setResultingType(
                    new SequenceType(
                            resultType.getArrayContentFacet(), SequenceType.Arity.OneOrZero));
            newContext.setSchema(arraySchema.elementType());
            newContext.setResultingQuery(
                    "get(" + newContext.getResultingQuery() + " ," + (this.lookup - 1) + ")");
        }
        return newContext;
    }
    ...
    public HomogeneousItemDataFrame getDataFrame(DynamicContext context) {
        ...
        boolean hasNonObjectJSONiqItem =
                isObject
                        && childDataFrame
                                .getItemType()
                                .getObjectKeysFacet()
        ...
        boolean hasRowIdColumn =
                Arrays.asList(fieldNames).contains(SparkSessionManager.rowIdColumnName);
        boolean hasMutabilityColumn =
                Arrays.asList(fieldNames).contains(SparkSessionManager.mutabilityLevelColumnName);
        boolean hasPathInColumn =
                Arrays.asList(fieldNames).contains(SparkSessionManager.pathInColumnName);

...
public class RumbleMLUtils {
    public static RuntimeDataFrame<Item> getDataFrameOrInferFromVariable(
        ...
        if (!context.getVariableValues().contains(inputVariableName)) {
            throw new OurBadException(
                    "ML input data is not available in the dynamic context", metadata);
        }

        if (context.getVariableValues().isDataFrame(inputVariableName, metadata)) {
            return context.getVariableValues()
                    .getDataFrameVariableValue(inputVariableName, metadata);
        }

        if (context.getVariableValues().isRDD(inputVariableName, metadata)) {
            JavaRDD<Item> rdd =
                    context.getVariableValues().getRDDVariableValue(inputVariableName, metadata);
            ItemType type =
                    TypeInferrenceUtils.inferItemTypeOfRDDItems(
                            rdd, metadata, TypeInferrenceUtils.TypeMergeMode.LAX);
            return ValidateTypeIterator.convertRDDToValidDataFrame(
                    rdd, type, context, true, staticContext);
        }

        List<Item> items =
                context.getVariableValues().getLocalVariableValue(inputVariableName, metadata);
        ItemType type =
                TypeInferrenceUtils.inferItemTypeOfLocalItems(
                        items, metadata, TypeInferrenceUtils.TypeMergeMode.LAX);
        return ValidateTypeIterator.convertLocalItemsToDataFrame(
                items, type, context, true, staticContext);
    }
    ...
    public static ParamMap convertRumbleObjectItemToSparkMLParamMap(
        ...
        for (int paramIndex = 0; paramIndex < paramMapItem.getStringKeys().size(); paramIndex++) {
            ...
            RumbleMLCatalog.validateTransformerParameterByName(
                    transformerShortName, paramName, metadata);
            String paramJavaTypeName =
                    RumbleMLCatalog.getJavaTypeNameOfParamByName(paramName, metadata);

            Object paramValueInJava =
                    convertParamItemToJava(paramName, paramValue, paramJavaTypeName, metadata);

            try {
                @SuppressWarnings("unchecked")
                Param<Object> sparkMLParam =
                        (Param<Object>)
                                transformer.getClass().getMethod(paramName).invoke(transformer);
    ...
    public static ParamMap convertRumbleObjectItemToSparkMLParamMap(
        ...
        for (int paramIndex = 0; paramIndex < paramMapItem.getStringKeys().size(); paramIndex++) {
            ...
            RumbleMLCatalog.validateEstimatorParameterByName(
                    estimatorShortName, paramName, metadata);
            String paramJavaTypeName =
                    RumbleMLCatalog.getJavaTypeNameOfParamByName(paramName, metadata);
            ...
            try {
                @SuppressWarnings("unchecked")
                Param<Object> sparkMLParam =
                        (Param<Object>) estimator.getClass().getMethod(paramName).invoke(estimator);
                result.put(sparkMLParam.w(paramValueInJava));
            } catch (NoSuchMethodException
                    | IllegalAccessException
                    | InvocationTargetException
    ...
    public static Object convertParamItemToJava(
            String paramName, Item param, String paramJavaTypeName, ExceptionMetadata metadata) {
        if (paramJavaTypeName.endsWith("[]")) {
            if (!(param.isArray())) {
                throw new InvalidArgumentTypeException(
                        paramName + " is expected to be an array type", metadata);
            }
            List<Object> paramAsListInJava = new ArrayList<>();
            // paramValue is expected to be an ArrayItem
            param.getItemMembers()
                    .forEach(
                            item -> {
                                paramAsListInJava.add(
                                        convertParamItemToJava(
                                                paramName,
                                                item,
                                                paramJavaTypeName.substring(
                                                        0, paramJavaTypeName.length() - 2),
                                                metadata));
                            });
            ...
            throw new InvalidArgumentTypeException(
                    paramName + " is expected to be an pipeline stage", metadata);
        } else {
            // complex SparkML parameters such as Estimator, Transformer, Classifier etc. are not
            // implemented yet
            throw new OurBadException(
                    "We have not implemented parameter support for type " + paramJavaTypeName);
        }
    }

    private static Object convertArrayListToPrimitiveArray(
            List<Object> paramAsListInJava, String paramJavaTypeName) {
        switch (paramJavaTypeName) {
            ...
            default:
                throw new OurBadException(
                        "Unhandled case of arrayList to primitive[] conversion found.");
        }
    }
    ...
    private static Object convertRumbleItemToJava(Item atomicItem, String javaTypeName) {
        ...
        switch (javaTypeName) {
            case "boolean":
                ...
                if (castItem == null) {
                    throw new OurBadException(
                            "We were not able to cast " + atomicItem + " to " + javaTypeName);
                }
            ...
            case "long":
                ...
                return castItem.getDecimalValue().longValue();
            default:
                throw new OurBadException(

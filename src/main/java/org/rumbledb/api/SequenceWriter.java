 ...
 *
 * <p>This class is effectively immutable: all configuration methods such as {@code mode()}, {@code
 * format()}, {@code option()}, etc. return a new {@link SequenceWriter} instance instead of
 * mutating the current one.
 *
 * <p>There are two mutually exclusive internal modes:
 *
 * <ul>
 *   <li>DataFrame mode: {@code dataFrameWriter != null} and {@code mode == null}. In this case, the
 *       sequence can be represented as a Spark {@link Dataset} / {@link DataFrameWriter} and is
 *       written using Spark's native writers (json/csv/parquet/...).
 *   <li>RDD mode: {@code dataFrameWriter == null} and {@code mode != null}. In this case, the
 *       sequence is serialized item-by-item via {@link Serializer} and saved as text files.
 * </ul>
 *
 * The serialization method (json, tyson, adaptive, xml-json-hybrid, yaml, delta, ...) is always
 * taken from {@code SerializationParameters#getMethod()}, which is the single source of truth for
 * the output format.
 */
public class SequenceWriter {
     ...
     *
     * <ul>
     *   <li>Either DataFrame mode: {@code dataFrameWriter != null} and {@code mode == null}.
     *   <li>Or RDD mode: {@code dataFrameWriter == null} and {@code mode != null}.
     *   <li>{@code serializationParameters} is never {@code null}.
     *   <li>{@code serializationParameters.getMethod()} is never {@code null}; serialization uses a
     *       predefined method.
     * </ul>
     */
    private SequenceWriter(
        ...
        if (dataFrameWriter == null && mode == null) {
            throw new OurBadException(
                    "Internal error: it is not possible for both the writer and the mode to be null");
        }
        if (dataFrameWriter != null && mode != null) {
            throw new OurBadException(
    ...
    /**
     * Public entry-point constructor used by {@link SequenceOfItems#write()}. TODO: update comment
     * here It determines the initial mode:
     *
     * <ul>
     *   <li>If the method is {@code adaptive}, {@code xml-json-hybrid}, or {@code tyson}, or if
     *       obtaining a DataFrame fails, the writer is created in RDD mode.
    ...
    SequenceWriter(SequenceOfItems sequence) {
        ...
        SerializationParameters params =
                sequence.getRuntimeStaticContext().getSerializationParameters();
    ...
    public SequenceWriter format(String source) {
        ...
        if (this.dataFrameWriter != null) {
            if (!source.equals("adaptive")
                    && !source.equals("xml-json-hybrid")
                    && !source.equals("tyson")) {
    ...
    public SequenceWriter options(java.util.Map<String, String> options) {
        ...
        DataFrameWriter<Row> newWriter =
                (this.dataFrameWriter != null) ? this.dataFrameWriter.options(options) : null;
        return createNewInstance(newWriter, this.mode, newParams);
    }
    ...
    public SequenceWriter partitionBy(String... colNames) {
        if (this.dataFrameWriter != null) {
            return createNewInstance(
                    this.dataFrameWriter.partitionBy(colNames), null, this.serializationParameters);
        } else {
    ...
    /**
     * Creates a new SequenceWriter instance with the specified components. This helper method
     * encapsulates the common pattern of creating new instances, reducing code duplication across
     * builder methods.
     *
     ...
     *   <li>append
     *   <li>ignore
     *   <li>error, errorifexists, default → ErrorIfExists
     * </ul>
     *

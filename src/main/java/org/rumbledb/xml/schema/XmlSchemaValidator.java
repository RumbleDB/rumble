...
public final class XmlSchemaValidator {
    ...
    /**
     * Creates a Xerces ValidatorHandler configured for the given root type and document validation mode.
     */
    ...
    /**
     * Validates that the input is an element or document node, and that a document node has exactly one element child.
     */
    private static Item validationElement(Item input, ExceptionMetadata metadata) {
        ...
        for (Item child : input.children()) {
            if (child.isElementNode()) {
                if (element != null) {
                    // A document node has more than one element child.
                    throw invalidDocument(metadata);
                }
                element = child;
            } else if (!child.isCommentNode() && !child.isProcessingInstructionNode()) {
                // A document node has a child that is neither an element, comment, nor processing instruction.
                throw invalidDocument(metadata);
            }
        }
        if (element == null) {
